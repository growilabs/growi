...
2. **Post one comment** on the tracking issue saying what was measured, what
   is missing, and which options are open.

   Before composing the comment, fetch this issue's occurrence frequency —
   the same count the Dashboard shows as its "First seen / Last seen /
   Occurrences" columns — and add it as one line in the comment body, above
   both of the fixed lines described below:

   ```bash
   node bin/flaky-ci/scripts/occurrence-summary.ts --issue "$ISSUE_NUMBER"
   ```

   On exit `0` this prints one line of JSON with `firstSeen`, `lastSeen`
   (each an ISO-8601 UTC timestamp, or `null` if no observation date could
   be parsed) and `occurrences` (a number). Render it as a single comment
   line, e.g.:

   ```
   - Occurrences: 5 (first seen 2026-09-10T03:00:00Z, last seen 2026-09-23T11:00:00Z)
   ```

   Where `firstSeen` or `lastSeen` is `null`, write `unknown` in its place
   instead of omitting the line — `occurrences` is always present as a
   number.

   If this script call fails (non-zero exit, or no output) — e.g. the issue
   or its comments could not be fetched — **omit the frequency line
   entirely** and post the comment as before. Do not retry and do not let
   this step block or delay posting the pause comment.

   Its **last two lines** are, in this order — and the frequency line, when
   present, always sits above both of them, never between them:
