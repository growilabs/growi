...
- [ ] Import specifiers in `apps/app/src` follow the no-extension convention (see below)

## Import Convention (apps/app/src)

All source files in `apps/app/src` follow a **single no-extension import convention** (since esm-import-convention, 2026-06):
...
**Never write `.js` or `.jsx` in source import specifiers.** Extensions are added by the post-build tool `bin/add-js-extensions.mjs` at server-build emit time, and `bin/verify-dist-resolution.mjs` confirms every emitted relative import resolves to an existing file.
...
// ✅ CORRECT: local (same src/ subtree) → relative
import { bar } from '../bar';

// ✅ CORRECT: external package → unchanged
import mongoose from 'mongoose';
```

Enforcement: `pnpm run lint:import-convention` (`tools/lint/import-extension-guard.cjs`).  
Batch migration codemod: `tools/codemod/normalize-import-convention.cjs`.
