 ...
 */
function fromGitTreeEntry(entry) {
 ...
 */
export async function writeBlob(content) {
  const repoPath = getRepoPath();
 ...
 * @returns 40-character SHA-1 OID.
 */
export async function writeTree(entries) {
  const repoPath = getRepoPath();
 ...
 * @returns 40-character SHA-1 OID.
 */
export async function writeCommit(opts) {
  const repoPath = getRepoPath();
 ...
 */
export async function readTree(oid) {
  const repoPath = getRepoPath();
 ...
 */
export async function updateRef(refPath, newOid) {
 ...
 */
export async function readRef(refPath) {
  const target = refFilePath(refPath);
  try {
 ...
 * @param refPath - Ref path relative to the git directory.
 */
export async function deleteRef(refPath) {
  const target = refFilePath(refPath);
  try {
...
//# sourceMappingURL=vault-repo-storage.js.map
