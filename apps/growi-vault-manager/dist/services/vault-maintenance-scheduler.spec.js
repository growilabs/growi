...
import { createVaultMaintenanceScheduler } from './vault-maintenance-scheduler.js';
 ...
 * { err } for failure.
 */
function mockExecFileSequence(responses) {
  let callIndex = 0;
 ...
 */
function mockCountObjects(count) {
 ...
 */
function mockGcSequence(before, after) {
  mockExecFileSequence([
 ...
 */
function findExecFileCall(subCommand) {
  return vi
    .mocked(childProcess.execFile)
    .mock.calls.find(
      (args) => Array.isArray(args[1]) && args[1].includes(subCommand),
    );
}
// ---------------------------------------------------------------------------
// Test suite
...
describe('VaultMaintenanceScheduler', () => {
  ...
  describe('in-flight namespace serialization', () => {
    it('does not squash a namespace that is already in-flight', async () => {
      ...
      let resolveSquash;
      const squashGate = new Promise((r) => {
        resolveSquash = r;
      });
      vi.mocked(VaultRepoStorage.readTree).mockResolvedValue([]);
...
//# sourceMappingURL=vault-maintenance-scheduler.spec.js.map
