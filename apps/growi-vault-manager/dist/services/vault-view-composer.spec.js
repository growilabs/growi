...
// Helpers
// ---------------------------------------------------------------------------
function makeUserView(overrides = {}) {
  return {
    userId: USER_ID,
...
beforeEach(() => {
  ...
  mockReadRef.mockImplementation(async (refPath) => {
...
describe('applyNamespaceDeltas', () => {
  ...
  it('re-reads only changed namespace refs from the repo', async () => {
    ...
    expect(refCalls.some((r) => r.includes('public'))).toBe(true);
    expect(refCalls.some((r) => r.includes('group-eng'))).toBe(true);
  });
});
// ---------------------------------------------------------------------------
...
describe('compose — conflict resolution (same path, multiple namespaces)', () => {
   ...
   */
  function setupNamespaceBlob(ns, commitOid, blobOid) {
    mockReadRef.mockImplementation(async (refPath) => {
  ...
  it('user-<uid>-only-me wins over public', async () => {
    ...
    let capturedWriteTreeCall = null;
    mockWriteTree.mockImplementation(async (entries) => {
      capturedWriteTreeCall = entries;
    ...
    const rootCall = allCalls.find((args) =>
      args[0].some((e) => e.path === 'page.md'),
    );
    expect(rootCall).toBeDefined();
    const pageEntry = rootCall[0].find((e) => e.path === 'page.md');
...
//# sourceMappingURL=vault-view-composer.spec.js.map
