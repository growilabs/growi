 ...
 */
const MANUAL_ENCODE_MAP: Record<string, string> = {
  '*': '%2A',
};
 ...
 */
function hasUpperCase(pagePath: string): boolean {
 ...
 */
function splitPagePath(pagePath: string): string[] {
 ...
 */
export function map(pagePath: string, pageId: string): string {
 ...
 */
export function mapPrefix(pagePath: string): string {
  const segments = splitPagePath(pagePath);
