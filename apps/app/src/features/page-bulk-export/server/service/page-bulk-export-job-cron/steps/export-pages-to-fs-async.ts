...
/**
 * Load Bootstrap CSS (shipped with @growi/app) and append fallback styles
 * for classless GFM tables generated by remark-gfm.  This ensures bulk-export
 * PDF/HTML output looks consistent with the web view.
 */
function getBootstrapCssForBulkExport(): string {
  try {
    const bootstrapCssPath = require.resolve('bootstrap/dist/css/bootstrap.min.css');
    return fs.readFileSync(bootstrapCssPath, 'utf8');
  }
  catch {
    return '';
  }
...
const bulkExportAdditionalCss = `
/* Apply Bootstrap table styling to classless GFM tables generated by remark-gfm. */
table {
  width: 100%;
  margin-bottom: 1rem;
  vertical-align: top;
...
const bulkExportPdfCss = getBootstrapCssForBulkExport() + '\n' + bulkExportAdditionalCss;

function wrapHtmlWithBulkExportStyles(htmlString: string): string {
  return `<style>${bulkExportPdfCss}</style>\n${htmlString}`;
}
...
async function convertMdToHtml(
  ...
  return wrapHtmlWithBulkExportStyles(htmlString);
}
