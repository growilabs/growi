 ...
 * independent of mastra's vendor selection (`mastra:llmVendor`): choosing a
 * non-OpenAI mastra vendor must NOT reroute or alter suggest-path's LLM path.
...
describe('callLlmForJson client selection', () => {
  ...
  it('should ignore mastra:llmVendor when selecting the client (independence from mastra)', async () => {
    // A non-OpenAI mastra vendor is configured; suggest-path must not be rerouted.
    mocks.configManagerMock.getConfig.mockImplementation((key: string) => {
      if (key === 'openai:serviceType') return 'openai';
      if (key === 'mastra:llmVendor') return 'anthropic';
      return undefined;
    });
    ...
    // ... and mastra:llmVendor is never consulted.
