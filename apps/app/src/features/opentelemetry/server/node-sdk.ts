...
import { setupCustomMetrics } from '~/features/opentelemetry/server/custom-metrics';
import { setResource } from '~/features/opentelemetry/server/node-sdk-resource';
...
`);
    ...
    if (isDev) {
      const { initLogger } = await import(
        '~/features/opentelemetry/server/logger'
      );
      initLogger();
    }
    ...
    const { generateNodeSDKConfiguration } = await import(
      '~/features/opentelemetry/server/node-sdk-configuration'
    );
