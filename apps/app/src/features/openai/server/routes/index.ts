...
export const factory = (crowi: Crowi): express.Router => {
  ...
  // enabled
    ...
    import('~/features/openai/server/routes/thread.js').then(({ createThreadHandlersFactory }) => {
      router.post('/thread', createThreadHandlersFactory(crowi));
    });

    import('~/features/openai/server/routes/get-recent-threads.js').then(({ getRecentThreadsFactory }) => {
      router.get('/threads/recent', getRecentThreadsFactory(crowi));
    });

    import('~/features/openai/server/routes/get-threads.js').then(({ getThreadsFactory }) => {
      router.get('/threads/:aiAssistantId', getThreadsFactory(crowi));
    });

    import('~/features/openai/server/routes/delete-thread.js').then(({ deleteThreadFactory }) => {
      router.delete(
    ...
    import('~/features/openai/server/routes/edit/index.js').then(({ postMessageToEditHandlersFactory }) => {
      router.post('/edit', postMessageToEditHandlersFactory(crowi));
    });

    import('~/features/openai/server/routes/ai-assistant.js').then(({ createAiAssistantFactory }) => {
      router.post('/ai-assistant', createAiAssistantFactory(crowi));
    });

    import('~/features/openai/server/routes/ai-assistants.js').then(({ getAiAssistantsFactory }) => {
      router.get('/ai-assistants', getAiAssistantsFactory(crowi));
    });

    import('~/features/openai/server/routes/update-ai-assistant.js').then(({ updateAiAssistantsFactory }) => {
    ...
    import('~/features/openai/server/routes/delete-ai-assistant.js').then(({ deleteAiAssistantsFactory }) => {
