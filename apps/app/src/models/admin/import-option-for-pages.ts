...
export class ImportOptionForPages extends GrowiArchiveImportOption {
  // `declare` keeps these type-only (no runtime field emit). Under
  ...
  // constructor assigned from initProps — which JSON.stringify then drops,
  // making the server-side `isImportOptionForPages` guard throw.
