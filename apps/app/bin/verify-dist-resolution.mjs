 ...
 *
 * @returns {{ checked: number, unresolved: string[] }}
 */
...
/**
 * @typedef {{ checked: number; unresolved: string[] }} VerifyDistResolutionResult
 */
 ...
 * @returns {VerifyDistResolutionResult}
 */
export function verifyDistResolution(distRoot) {
  let checked = 0;
  /** @type {string[]} */
  const unresolved = [];
...
// Allow direct CLI invocation: node verify-dist-resolution.mjs <distRoot>
if (process.argv[1] === new URL(import.meta.url).pathname) {
  const distRoot = process.argv[2];
  if (!distRoot) {
    // biome-ignore lint/suspicious/noConsole: build script diagnostic
    console.error('Usage: node verify-dist-resolution.mjs <distRoot>');
    process.exit(1);
  }
