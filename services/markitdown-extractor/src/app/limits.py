...
1. Concurrency control (Req 2.3): a global asyncio.Semaphore limits the number
   ...
   available the caller receives ServiceBusy immediately instead of queuing.
...
3. Extraction timeout (Req 2.2): with_timeout wraps an awaitable in
   asyncio.wait_for and converts asyncio.TimeoutError into ExtractionTimeout
...
class Limits:
    ...
    async def enforce_size(self, data: bytes) -> None:
        ...
        Raises:
            FileTooLarge: When ``len(data)`` exceeds the configured limit.
        """
        if len(data) > self._max_bytes:
            raise FileTooLarge(
                f"File size {len(data)} bytes exceeds the {self._max_file_size_mb} MiB limit"
            )
    ...
    async def with_timeout(self, coro: Awaitable[T]) -> T:
        ...
        :class:`ExtractionTimeout` so callers never have to handle the
        ...
        Raises:
            ExtractionTimeout: When *coro* does not complete within
                ``timeout_s`` seconds.
        """
        try:
            ...
            raise ExtractionTimeout(
                f"Extraction timed out after {self._timeout_s}s"
            )
