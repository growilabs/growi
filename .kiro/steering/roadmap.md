...
- **Shared seams to watch**:
  - **コマンドレジストリの所有権**: 現状 `editor-slash-command` がレジストリを所有。`editor-selection-palette` がこれを共有する際、レジストリを基盤の共通モジュールへ昇格させるか slash 配下のまま参照させるかを設計で確定する（共有所有の曖昧化を避ける）。
...
- [ ] editor-selection-palette -- テキスト選択時に浮動コマンドパレット（バブルメニュー）を表示し、選択範囲への操作（太字・斜体・取り消し線・インラインコード・リンク化等）を提供。Dependencies: editor-slash-command
