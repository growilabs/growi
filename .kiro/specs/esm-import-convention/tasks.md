...
- [ ] 1. server ビルドの解決モードをゆるい解決へ移行し型シムを棚卸し
...
- [ ] 2. 出力時 `.js` 自動付与ツールをテスト駆動で整備 (P)
- [ ] 2.1 付与ロジックの fixture テストを先に書く（red）
...
- [ ] 2.2 付与ツールを実装し postbuild に組み込む（green）
...
- [ ] 3. 出力物の全 import 静的網羅解決検証ツールを整備 (P)
...
- [ ] 4. 一括移行 codemod をテスト駆動で整備 (P)
- [ ] 4.1 変換ロジックの fixture テストを先に書き master で較正（red）
...
- [ ] 5. codemod を apps/app/src 全体へ適用する
...
- [ ] 10. 単一規約をドキュメント/steering に明文化する
