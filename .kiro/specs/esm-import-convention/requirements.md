...
本 spec は、apps/app/src の import 記法を**単一の「拡張子なし」規約**（local = 相対 `./X` / `../X`、cross-module = `~/X` alias）に統一し、Node ネイティブ ESM が要求する `.js` 拡張子は**ソースではなく server ビルドの emit 時にのみ**付与する。これにより記法選択から `.js` と「プログラム所属の推測」を排除する。ランタイムの挙動・bundling 戦略は変更しない。
...
#### Acceptance Criteria
1. The import-convention は、同一/子孫/祖先/兄弟ディレクトリのモジュール（local）参照を**拡張子なしの相対 specifier**（`./X` / `../X`）で表現するものとする。
2. The import-convention は、ディレクトリ区分を跨ぐ（cross-module）参照を**拡張子なしの `~/` alias specifier**（`~/X`）で表現するものとする。
...
2. When 移行ツールを実行するとき, the migration tool は local 参照に過剰適用された `~/...` alias を拡張子なしの相対 specifier へ戻すものとする。
