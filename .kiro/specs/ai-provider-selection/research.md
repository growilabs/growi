# Gap Analysis: ai-provider-selection

_生成日: 2026-06-10 / 対象要件: requirements.md (Requirement 1–5)_

> 後続整合（ai-provider-multi-model）: 本 research の各 D-セクションは当時の設計判断の履歴である。以降の `ai:model`（単一）/ `ai:providerOptions`（グローバル単一）/ `requireModel()` / `getModel()` への言及は、その後 ai-provider-multi-model で **`ai:allowedModels`（モデル + per-model providerOptions + isDefault）+ `resolveEffectiveModelId(modelId?)` / `getDefaultModelId()` / `getAllowedModels()` / `getProviderOptionsForModel(effectiveModelId)`** へ統合・置換されたものとして読む（env `AI_MODEL` / `AI_PROVIDER_OPTIONS` は廃止、自動移行なし）。ベンダー（プロバイダ）切替は引き続き本 spec のスコープ外で、モデル単位の per-request 選択は ai-provider-multi-model が扱う（同一プロバイダ内）。
...
> **注記（`ai-provider-model-picker` との関係）**
>
> 別スペック `ai-provider-model-picker` が models.dev を利用するが、D-2/D-3 の決定と**矛盾しない**。両者は目的も通信特性も異なる別経路である。
>
...
> 2. **vendored 静的 read は別物**: `ai-provider-model-picker` が採用した「models.dev を**取り込みステップ（リリース前段）**で vendoring し、chat＋tool 対応に絞って**コミットした静的カタログ**を実行時に read する」経路は、D-2 の runtime fetch とは**別物**。相違点は以下:
...
>    - Cross-reference: 詳細は `.kiro/specs/ai-provider-model-picker`。
