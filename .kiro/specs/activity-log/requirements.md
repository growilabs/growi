...
このドキュメントは、activity log（監査ログ）の**記録ゲート**、すなわち「どの操作を activity レコードとして永続化するか」の制御に関する要件を定義する。目的は、記録対象外の action が更新系（非 GET）経路で DB に残る現状を解消し、記録対象外の操作を今後永続化しないようにすることである。activity log は GROWI 全体（通知・貢献度グラフ・監査／コンプライアンス）で広く使われるため、記録抑制の変更は既存の記録・通知・集計の観察可能な挙動を壊してはならない。
...
- **Adjacent expectations**:
  - 記録抑制の「方式」（今後の書き込み自体を減らす／確定後に残骸を消す）と、確定イベントが送出されない操作（action が未確定のまま終わる）の扱いは design で決定する。方式により MongoDB への書き込み回数の削減度合いと、未確定操作の残骸の扱いが異なる。
