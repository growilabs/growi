...
1. **決定 3（検索結果を並べるか、まとめるか）を決める** — 残る唯一の大きな判断。
   まとめるなら、まとめる処理をどこに置くかも一緒に決める。決定 4（ページ作成）はこれに従属する。
   決定 8（Gen 1 をいつまで動かすか）は独立に決められる。
2. **決定 6 の毎回のリクエスト認証（RFC 9421 + Ed25519）は設計を先に確定してよい。**
3. **閉域向け推奨構成のドキュメントを起こす**（決定 7）。Teams のセットアップ手順（決定 2）と内容が重なるのでまとめて作る。
4. 決定 3 が決まったら `/kiro-spec-init` で Gen 2 の spec を起こす。本ブリーフはその入力になる。
5. 並行して、リスクの実測を取る:
