...
**決める制約**: 「GROWI が inbound 公開されていない環境をサポート対象にするか」。
サポートするなら G-2 か G-3。しない（＝ Gen 1 と同じ前提を続ける）なら G-1 で、
実装量も運用の複雑さも最小に収まる。ここは推測ではなく **self-host 利用者の実態調査で決めるべき**。
...
1. **self-host 利用者の実態を調べる。** 決定 G（inbound 公開の有無）と決定 H（公式ホスト版の利用状況）の
   両方がこの一次情報に依存しており、推測で決めると後で覆る。**ここが最優先。**
2. **決定 C を決める**（串刺しを配送レベルに留めるか、統合レベルへ進めるか。進めるなら統合層をどこに置くか）。
   D はこれに従属する。B は独立に決められる。
3. **決定 F の層②（RFC 9421 + Ed25519）は他に依存しないので、設計を先に確定してよい。**
   残る変数の鍵配布方式は決定 G から従属して決まる。
