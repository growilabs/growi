...
- 案 1: 第一級サポート（ドキュメント・セットアップ UI まで用意）
- 案 2: 「上級者向け」として対応はするが導線は用意しない
- 案 3: Gen 2 初版のスコープ外

**決める材料**: Teams を求めているのがどの顧客セグメントか。大企業からの要望が中心なら、案 1（本格サポート）のコストは見合う。

### 決定 3: 複数 GROWI の検索結果を、そのまま並べるか 1 つにまとめるか
...
| | 案 1: そのまま並べる（fan-out） | 案 2: 1 つにまとめる（統合・順位付け・要約） |
|---|---|---|
...
逆に言えば、そのまま並べる案の範囲なら LLM は最後まで不要。

1 つにまとめる案を選んだ場合でも、**まとめる処理をプロキシに置くとは限らない**:

- **案 2a: 集約役の GROWI が担う** — 1 台の GROWI が他の GROWI 向けのスコープ付きアクセストークン
...
- **案 2b: プロキシが担う** — プロキシが GROWI 全台分の API 資格情報と、利用者ごとの LLM の API キーを保持し、
  自らエージェントを動かす。今はトークンペアしか持たないコンポーネントに、
  **巨大な信頼・課金の面**が生える。

**決める材料**: 「LLM を使うか」ではなく **「N 台分の ACL 統合責任を誰が負うか」**。
プロキシに負わせたくないなら、そのまま並べる案か案 2a。案 2b を選ぶ理由は
...
**決める材料**: 決定 3 と同じ論点（書き込み権限を誰が持つか）。決定 3 が決まればこちらも決まる。
...
残る変数は公開鍵の配り方で、これは決定 7（どちらから接続しに行くか）が決まれば自動的に決まる。
(c) の「誰の代理か」は決定 3・4 次第 — 検索結果をそのまま並べる案なら当面不要。

### 決定 7: proxy と GROWI のどちらから接続しに行くか
...
| | 案 1: proxy から GROWI へ（Gen 1 と同じ） | 案 2: GROWI から proxy へ常時接続 | 案 3: 両対応 |
|---|---|---|---|
| 方向 | proxy が各 GROWI へ POST | **GROWI がプロキシへ接続しに行き、その接続を保持し続ける** | 到達できるなら案 1、できないなら案 2 |
...
**決定 6（お互いを本物だと確認する方法）と密接に関係する**: 案 2 を採ると、公開鍵をどう配るかという論点が**消える**。
接続確立時のハンドシェイクで相互認証すれば、以後そのチャネルは認証済みとして扱えるため、
リクエストごとの RFC 9421 署名も原理的には不要になる（監査や多段プロキシ対策として残す選択はあり得る）。
逆に案 1 なら、決定 6 に書いたとおり **ペアリング時に公開鍵そのものを登録する方式**にする必要がある。

#### 推奨: 案 2（GROWI から proxy へ常時接続）
...
1. **決定 7（どちらから接続しに行くか）を決める** — 社内ネットワークからしか見えない GROWI をサポート対象に含めるか、という製品判断。
   含めるなら「GROWI から proxy へ接続」案を推奨。これが決まると決定 6 の公開鍵の配り方も自動的に決まる。
2. **決定 3（検索結果を並べるか、まとめるか）を決める** — まとめるなら、まとめる処理をどこに置くかも一緒に決める。
   決定 4（ページ作成）はこれに従属する。決定 2（Teams）と決定 8（Gen 1 の扱い）は独立に決められる。
3. **決定 6 のうち毎回のリクエスト認証（RFC 9421 + Ed25519）は他に依存しないので、設計を先に確定してよい。**
4. 決定 3 と決定 7 が決まったら `/kiro-spec-init` で Gen 2 の spec を起こす。本ブリーフはその入力になる。
5. 並行して、リスクの実測を取る:
...
6. `@growi/chat`（チャットサービスに依存しないメッセージ形式）のインタフェース草案を書く。**Gen 2 の工数はここに最も集中する。**
