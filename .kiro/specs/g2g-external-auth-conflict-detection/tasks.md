...
- 8.1: `externalusergroups`は取り込み中のデータ不変性(要件2.3)を確認するsnapshot的な試験がどこにも無い(`externalaccounts`は`g2g-transfer.integ.ts`にある)。8.2の要件は4.1-4.3で別範囲のため、ここで拾わず`/kiro-validate-impl`実行時にフィーチャレベルの懸念として報告すること。
