...
| effective-model-catalog | Server (runtime) | `getEffectiveSelectableModelIds(provider)` = 更新済み／同梱の**新しい方**（`fetchedAt` vs 同梱 `_generatedAt` の比較。同梱が**厳密に**新しい場合のみ同梱＝イメージ更新後の古いスナップショットの覆い隠しを防止。tie・タイムスタンプ不正は更新済み優先。無ければ同梱。`?? []` フェイルソフトは従来どおり）。get-available-models はこれを await する | 9.5, 2.x, 3.1 |
