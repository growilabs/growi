...
- Web/クライアントレンダラ（`services/renderer`, `client/services/renderer`）の挙動変更。ただし
  ドリフトテストが `generateCommonOptions` の選定を **読み取り参照** することは許可。
...
- Web `generateCommonOptions` の **プラグイン集合・順序**の変更（ドリフトテストが検知）。
...
graph TB
  ...
  subgraph webRenderer[services renderer read only]
    Common[generateCommonOptions plugin selection]
  end
...
| Styling | `@growi/core-styles` + `_wiki.scss`（ビルド時プリコンパイル CSS）+ KaTeX CSS | 出力スタイル | vendor-styles プリコンパイル方式に準拠 |
...
| RendererParityGuard | test | Web プラグイン集合との乖離検知 | 1.6,6.1,6.2 | generateCommonOptions(read,P1) | — |
...
- Risk: SCSS→CSS プリコンパイル機構と `dependencies` 分類（tech.md、Turbopack 外部化）はタスクで確定。

### test / RendererParityGuard
- `plugin-set.ts` の採用集合と、`generateCommonOptions` の remark/rehype 選定を突き合わせ、Web 側の
  各プラグインが bulk-export 側で **included / intentionally-excluded** のいずれかに分類済みである
  ことを検査。未分類の新規プラグイン出現で失敗（6.1, 6.2）。意図的除外一覧は Technology Stack 脚注
  と整合させる。
...
- **XSS / 危険コンテンツ**: rehype-sanitize を必須実行。allowlist は Web 側 `getCommonSanitizeOption`
  相当 ＋ in-scope 追加（数式コンテナ等）に限定。
...
- **R-css**: 注入 CSS の SCSS→CSS プリコンパイル機構・`dependencies` 分類・deploy 検証（tech.md）。
  `_wiki.scss` は `@growi/core-styles` の bootstrap utilities/variables に依存するため、自己完結する
  エントリ SCSS を用意してコンパイルする必要がある。
- **R-katex（確定）**: 数式は Phase 1 に含める（ユーザー決定）。KaTeX CSS をプリコンパイル CSS に
  同梱する（タスク 1.1）。同梱が無いと数式が崩れる点に留意。
