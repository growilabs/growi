...
> 2. **プラグイン宣言の単一出所化**: 採用プラグインの列挙を `plugin-set.ts` の 1 箇所に集約。`EsmPluginLoader`
>    は `ADOPTED_PLUGINS` を走査して `dynamicImport` し、`BulkExportMarkdownRenderer` は読み込み順に
>    `.use()` する。プラグイン追加は **plugin-set.ts の 1 ファイル編集のみ**（loader/renderer は不変）。
...
export interface LoadedPlugin {
  readonly name: string;                       // canonical npm name (= plugin-set.ts entry)
  readonly plugin: import('unified').Plugin;   // default export, ready for processor.use()
...
export interface LoadedPipeline {
  ...
  readonly plugins: readonly LoadedPlugin[];   // ordered exactly as ADOPTED_PLUGINS
}

export function loadPlugins(baseDir: string): Promise<LoadedPipeline>;
```
- 改訂 2: ハードコードの名前付きフィールドを廃止し、`ADOPTED_PLUGINS`（plugin-set.ts）を走査して
  `dynamicImport` した**順序付きリスト**を返す。プラグイン追加は plugin-set.ts のみで完結し、本 loader と
  renderer は不変。旧来の「loader と plugin-set の逆方向アサーション」は不要になり削除。
