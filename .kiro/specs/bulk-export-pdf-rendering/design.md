...
`.wiki { blockquote {…} h1..h6 {…} }` のように **素の要素**に当たるよう書かれているため、引用・見出し・
コード等はクラス付与なしで装飾される。

> **訂正（改訂 4, 2026-06-11）**: 当初の前提（research.md I-table:「表もクラス付与不要、`.wiki table` で
> 枠線が当たる」）は**誤り**だった。`.wiki table` は `font-size` のみで枠線を持たず、GROWI の表の枠線は
> Web レンダラの `add-class` が付与する **`table table-bordered`（Bootstrap クラス）由来**である。実機 PDF で
> 表だけ無装飾になったため、`<table>` に `table table-bordered` を付与する。**実装は Web の `add-class`
> プラグインを再利用する**（`add-class.ts` の実行時依存は `hast-util-select`(ESM、既存の production
> dependency)のみで、`recommended-whitelist.ts` と同じ CJS→ESM 再利用パターン）。**add-class は `plugin-set.ts`
> の `ADOPTED_PLUGINS` に正規の 1 エントリとして宣言**し（`specifier`=相対パス, `exportName`='rehypePlugin',
> `options`=`{ table: 'table table-bordered' }`、sanitize の後・stringify の前に配置）、loader が汎用的に
> ロードする。これによりローカル再実装も位置の特殊分岐も持たず、生成済み Bootstrap 表 CSS を再利用して Web と
> 同一描画になる（手書き表 CSS なし）。表以外（引用/見出し/コード等）は従来どおり素要素＋`.wiki` CSS で装飾される。

本設計は現行 CJS サーバランタイム前提で閉じる（Web レンダラ本体・ローカル .ts プラグインは
`ERR_REQUIRE_ESM` で import 不可、research.md I2）。

> **実装改訂ノート（post-impl, 2026-06-11）**: レビュー指摘を受けた 3 点の改訂。詳細は各節へ反映済み。
> 1. **CSS の重複排除（per-page → per-job）**: CSS を各ページに `<style>` でインラインせず、job ごとに
>    共有スタイルシート `_bulk-export.css` を 1 ファイル書き出し、各ページは相対 `<link rel="stylesheet">`
>    で参照する。pdf-converter は `setContent` をやめ `page.goto(file://…)` でページを読むため相対参照が解決
>    する。さらに KaTeX フォントは **woff2 のみ**インライン化（woff/ttf を破棄、~1.8MB→~757KB）。これにより
>    1 ページあたりの HTML は `<link>` 1 行＋本文のみになる。**この改訂で pdf-converter の HTML 読み込み
>    機構が本 spec の owns に入る**（下記 Boundary 改訂）。
> 2. **プラグイン宣言の単一出所化**: 採用プラグインの列挙を `plugin-set.ts` の 1 箇所に集約。`BulkExportMarkdownRenderer`
>    が `loadPlugins(baseDir, ADOPTED_PLUGINS)` で宣言を渡し、`EsmPluginLoader` は**与えられた宣言を
>    ロードするだけ**（何をロードするかは知らない＝責務分離）。renderer は読み込み順に `.use()` する。プラグイン
>    追加は **plugin-set.ts の 1 ファイル編集のみ**（loader/renderer は不変）。`PluginDeclaration` は
>    `name`（parity 用の正規名）/ `specifier`（npm 名 or 相対パス）/ `exportName`（default 以外）/ `options`
>    を持ち、npm プラグインもローカル再利用プラグインも同一の宣言で表現する。
> 3. **ドリフトテストの堅牢化**: `renderer.tsx` の正規表現スクレイピングをやめ、TypeScript コンパイラ API で
>    **import 宣言を AST 抽出**する方式へ。手書きブロックリストを撤廃し、解析不能時は loud に失敗する。

> **改訂 5（2026-06-12）— React 非依存プラグインの追加採用**: research.md I2 の「ローカル .ts プラグインは
> `ERR_REQUIRE_ESM` で読めない」という断定は **`renderer.tsx` の静的 import に対してのみ真**であり、
> **個別プラグインファイルを `dynamicImport` する経路では成立しない**（改訂 4 で add-class を相対パス
> `dynamicImport` で本番ロードできた実例が反証）。実体が **React/DOM 非依存の純粋な AST 変換**であるローカル
> プラグインは add-class と同一パターンで再利用できる。これを踏まえ、以下を `ADOPTED_PLUGINS` に追加採用する:
> - **emoji**（remark, ローカル）: `:smile:` 等 → 絵文字グリフ。依存は `@growi/emoji-mart-data`(ESM) /
>   `mdast-util-find-and-replace`(ESM) のみ。
> - **xsv-to-table**（remark, ローカル）: CSV/TSV コードブロック → 表。採用済み add-class が `table table-bordered`
>   を付与するため枠線まで Web と揃う。
> - **remark-directive**（remark, npm）+ **echo-directive**（remark, ローカル）: text/leaf ディレクティブを
>   可読テキスト（`<span>`/`<div>` ＋ 記法表示）へ劣化。属性構文 `{...}` を生のまま露出させない。
>
> いずれも Web の `generateCommonOptions` が既に採用するプラグインで、SSR 経路で本番にバンドル済み
> （新規 npm 依存・新規 `dependencies` 分類は不要）。**`remark-github-admonitions-to-directives` は採用しない**:
> callout（React）不在では `> [!NOTE]` を匿名 `<div>` に変換してアラート種別ラベルを失い、blockquote のままより
> 劣化が悪化する実測結果のため（research.md I7）。callout 描画・mermaid/drawio/lsx/plantuml は引き続き Phase 2。
...
- **`remark-github-admonitions-to-directives` の採用**（改訂 5）。callout 不在では `> [!NOTE]` を匿名
...
- ローカル .ts プラグインの再実装（コピー）や独自インライン変換。**再利用は可**（改訂 5 で emoji /
  xsv-to-table / echo-directive を add-class と同一の相対パス `dynamicImport` で再利用する。再実装ではなく
  Web と単一の実装を共有する）。
...
| Backend / Services | emoji（ローカル）, xsv-to-table（ローカル） | 絵文字グリフ化 / CSV・TSV→表（改訂 5） | 相対パス `dynamicImport`・React/DOM 非依存。Web `generateCommonOptions`/SSR と同一実装を再利用 |
| Backend / Services | remark-directive（npm）, echo-directive（ローカル） | ディレクティブ解析＋text/leaf を可読テキストへ劣化（改訂 5） | echo は container を扱わない（callout の領分）。container は内部テキスト保持の素ブロックに劣化 |
...
> **採用（ADOPTED_PLUGINS）**: remark-gfm, emoji（改訂 5）, remark-directive（改訂 5）, echo-directive
> （改訂 5）, remark-frontmatter, remark-math, xsv-to-table（改訂 5）, remark-rehype, rehype-raw, rehype-slug,
> rehype-sanitize, rehype-katex, add-class（改訂 4）, rehype-stringify。
>
> **意図的除外（理由は 2 区分。research.md I2/I7 参照）**:
> - *callout 不在では劣化が悪化*: `github-admonitions`（`> [!NOTE]` → 匿名 div、ラベル喪失。改訂 5）。
...
>
> ※ いずれも「読み込めないから除外」ではない（改訂 5 で訂正）。純粋な AST 変換プラグインは add-class と同じ
...
- `…/steps/export-pages-to-fs-async.ts` — `convertMdToHtml` / `wrapHtmlWithBulkExportStyles` /
  ...
  resume・エラー処理は維持。改訂 1: `getPageWritable` を async 化し、pdf 形式では job 開始時に共有
  `_bulk-export.css` を 1 回書き出す。各ページは相対 `cssHref`（`toCssHref`）を算出して `renderToHtml` に渡す。
- `apps/pdf-converter/src/service/pdf-convert.ts`（改訂 1, 越境）— cluster task を `setContent` から
...
> 改訂 1: 共有 CSS（`_bulk-export.css`）は job ごとに 1 回だけ書き出し、各ページ HTML は相対 `<link>` で
...
- ツリー変換は最小限。原則 **素の HTML 要素**を出力し `.wiki` CSS で装飾するが、**例外として `<table>` には
  `table table-bordered` を付与**する（改訂 4: 素の表には Bootstrap の枠線が当たらないため。Web の `add-class`
...
- text/leaf ディレクティブは remark-directive で解析し echo-directive で記法を可読テキスト化（属性
  ...
  内部テキストを保持した素ブロックへ劣化（3.1）。GitHub アラートは blockquote のまま（admonitions 不採用、改訂 5）。
...
export interface BulkExportMarkdownRenderer {
  /** Shared CSS to write once per job; pages link to it (改訂 1). */
  getCss(): string;
  /**
...
- パイプライン組み立ては `plugin-set.ts`（`ADOPTED_PLUGINS`）を読み込み順に `.use()` する（改訂 2）。
...
```
- 改訂 2: ハードコードの名前付きフィールドを廃止し、**呼び出し元が渡した宣言**を走査して `dynamicImport` した
...
- Constraints: CSS は `_wiki.scss`（`.wiki` スコープ）＋ `@growi/core-styles` ＋ KaTeX CSS を
  ビルド時にプリコンパイルした静的文字列。テーマ/レイアウト/chrome は含めない（2.3）。改訂 1: `wrap()` は
...
- `plugin-set.ts` の採用集合と、Web 側 `renderer.tsx`（`generateCommonOptions` と `generateSSRViewOptions`
  ...
  **included / intentionally-excluded** のいずれかに分類済みであることを検査。さらに bulk-export が採用する
  ...
  プラグイン出現で失敗（6.1, 6.2）。
- 改訂 3: 抽出は **TypeScript コンパイラ API による AST 解析**で行う（`renderer.tsx` を `createSourceFile`
...
- **エスケープ無効化禁止（4.2）**: pdf-converter が `--no-sandbox` で動作するため、raw HTML を
  ...
  必ず sanitize する。
- **ディレクティブ属性の無害化（4.3, 改訂 5）**: echo-directive は text/leaf ディレクティブの属性
...
- **R-katex（確定）**: 数式は Phase 1 に含める（ユーザー決定）。KaTeX CSS をプリコンパイル CSS に同梱し、
  **フォントは `@font-face` の `src` を base64 data URI でインライン化**する（外部 `url()` 参照を残さず
  自己完結。タスク 1.1, 1.2）。改訂 1: Chromium が解せる **woff2 のみ**を同梱し、woff/ttf 代替は破棄する
  （payload の約 2/3 削減）。共有スタイルシート化により、フォント payload は job あたり 1 回のみ存在する。
- **R-dedup-css（改訂 1, 確定）**: CSS をページにインラインすると N ページぶん重複する。これを避けるため
