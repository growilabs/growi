...
- GROWI ローカル .ts プラグイン（emoji / xsv-to-table / echo-directive 等）の再利用。ただし **add-class は例外**
  で再利用する（改訂 4: 依存が `hast-util-select` ESM のみで dynamicImport 可能。表の枠線に必須）。
...
> echo-directive, codeblock, xsv-to-table, github-admonitions, callout, add-inline-code,
> relative-links。理由は research.md（ローカル .ts / React 依存 / コールアウト非ゴール）。
> ※ **add-class は採用**（改訂 4、ADOPTED_PLUGINS に宣言）。`hast-util-select` ESM のみ依存で dynamicImport
> 可能、表の枠線（`table table-bordered`）に必須のため。
