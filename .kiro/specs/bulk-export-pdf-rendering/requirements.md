...
- **Out of scope**:
  ...
  - GitHub アラート / GROWI ディレクティブ（`:::note` 等）の **装飾コールアウト描画**。
    色付きコールアウトの見た目は React コンポーネント駆動のため本 spec では再現せず、アラートは
    blockquote、ディレクティブは可読テキストへ劣化させる（将来フェーズ renderer-convergence）
  - GROWI ローカルの Markdown 拡張に依存する機能: 絵文字ショートコード変換 / CSV・TSV→表 /
    コードのシンタックスハイライト配色 / drawio・plantuml・lsx・attachment-refs のライブ描画
    （いずれも ESM 化後の将来フェーズ）
...
#### Acceptance Criteria
1. If ページ本文にスコープ外機能に依存する記法（GROWI ディレクティブ `:::note` 等・絵文字ショートコード・CSV/TSV コードブロック・drawio/plantuml/lsx 等）が含まれる, then the Bulk Export Renderer shall エクスポートを失敗させず、当該箇所を可読なテキストまたはコードブロックとして出力する。
