...
**この「単一の判定基盤」という方向性は、GROWI コミュニティ側からも既に提案されている。**
[discussion #7709「管理者以外によるグループの設定」](https://github.com/growilabs/growi/discussions/7709)
で、コアメンテナー自身が「**アビリティ**」という概念——「ユーザーグループの設定可」「カスタマイズの
設定可」のような capability を**複数個、ユーザーまたはグループに付与できる**——を提案しており、
別の参加者はこれを「**同一の仕組みでページの閲覧・編集権限も管理できる**のでは」と発展させている。
これは本 spec の Role × Capability モデル（ユーザー／グループ双方へ付与、`page:*` は既存判定へ
委譲）と方向性が一致する、数年越しの実要望である。
