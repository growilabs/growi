...
- Task 5 (live visual check, performed by the user): confirmed a real Requirement 3.2/3.3 overflow bug on long CJK ancestor segments — `SearchResultAncestorPath` was placed as a bare flex child of `PageListItemL`'s `d-flex justify-content-between` row with no `min-width: 0` override at that specific nesting level, so an unbroken long segment could overflow the row instead of triggering the component's own per-segment ellipsis (classic flexbox `min-width: auto` default). Fixed by wrapping it in a `flex-grow-1` span with an explicit `style={{ minWidth: 0 }}`, mirroring the already-working pattern used for `SearchResultPagePath` in `SearchResultMenuItem.tsx`. Confirmed via unit tests + code-level tracing; re-verify visually. **Anyone adding a truncating flex child inside an existing row (here or in a future phase per roadmap.md) must add this same explicit `min-width: 0` wrapper at the exact flex-item boundary — setting it only inside the truncating component's own CSS module is not sufficient if the immediate JSX parent doesn't also allow shrinking.**
