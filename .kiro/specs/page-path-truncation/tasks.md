...
- [x] 4. Integration: `PageListItemL` のオプトイン配線と `/_search` への適用
...
- Task 5: this devcontainer's seeded DB has guest access disabled (anonymous `/_search` returns `302` to `/login`), so early attempts assumed authenticated verification was also blocked by a sandbox session-cookie restriction — **a follow-up investigation showed this was a misdiagnosis**: `curl` with a cookie jar can log in (`POST /_api/v3/login`, 200) and fetch `/_search?q=...` authenticated (200) with no issue. The real, confirmed blocker is architectural: `/_search` fetches its result rows client-side via SWR/Elasticsearch after hydration, so `curl`/any HTTP-only tool only ever sees the SSR shell (`__NEXT_DATA__`), never the rendered ancestor-path markup, regardless of auth. Seeing it requires a JS-executing browser (e.g. Playwright), and no browser binary or browser-automation tool was available in this sandbox. All other requirements are covered by the 50 passing unit/component tests plus `turbo run lint/test/build` (lint's only failure is a pre-existing, gitignored, unrelated vendored-plugin config issue under `apps/app/tmp/`).
- Task 5 (live visual check, performed by the user, twice): confirmed a real Requirement 3.2/3.3 overflow bug on long CJK ancestor segments. **This needed two rounds of fixes, not one** — a genuine multi-level flexbox `min-width: auto` chain, confirmed by installing headless Chromium (Playwright) in-session and walking the actual computed-style ancestor chain (not theorizing):
  1. First fix (insufficient on its own): wrapped `SearchResultAncestorPath` in a `flex-grow-1` span with `style={{ minWidth: 0 }}`, mirroring `SearchResultPagePath`'s wrapper in `SearchResultMenuItem.tsx`. This was necessary but the row still overflowed — confirmed via the user's screenshot, then independently reproduced and measured (`scrollWidth`/`clientWidth` at every ancestor level via a headless-browser script).
  2. Second fix (the actual root cause): the pre-existing `<div className="flex-grow-1 px-2 px-md-4">` that wraps the whole path/title/snippet block for every `PageListItemL` row is *itself* a flex item of the row's outer `d-flex` container, and it also defaulted to `min-width: auto`. It had never needed `min-width: 0` before because the old `PagePathHierarchicalLink` + `Clamp` content always wrapped across multiple lines instead of needing single-line horizontal shrink — `SearchResultAncestorPath` was the first content in this row to actually require it. Fixed by setting `style={{ minWidth: 0 }}` on that div, but **only when `isPathTruncationEnabled` is true**, so the two untouched consumers stay byte-for-byte unchanged.
  - **Lesson**: a `min-width: 0` fix on the truncating element's immediate wrapper is not enough — verify by measuring (or visually confirming) every ancestor level up to the actual scrollable/visible boundary, since *any* intermediate flex item without `min-width: 0` breaks the chain, even ones that pre-date the feature and were never touched before.
  - **Tooling note**: `npx playwright install chromium` succeeded in this devcontainer (network access was fine) after the earlier claim in this same file that "no browser binary was available" — that claim was about *not yet having tried*, not a real environment limitation. Installed to `/home/vscode/.cache/ms-playwright`; usable via `require('/workspace/growi/node_modules/.pnpm/playwright-core@<version>/node_modules/playwright-core')` (not hoisted to workspace-root `node_modules` under this pnpm layout). Keep any such verification script **outside** `apps/app/` — nodemon watches `*.*` there and restarts the whole dev server (with a crash-loop if a second `pnpm run dev` is started against an already-running one) on every file add/edit; use the session scratchpad directory instead.
- PR #11743 review fixes (after task 5):
  - **Highlight alignment is now per segment, in one place** (`client/util/align-highlighted-path-segments.ts`, used by both `buildAncestorPathNodes` and the page-name row). The earlier page-name guard (`isPageNameHighlightReliable`, which counted `/`-split segments of each `former`) miscounted the `/` inside every `</em>`, so *any* ancestor highlight dropped the page-name highlight (Requirement 6.1 regression). Re-running `DevidedPagePath` on the highlighted string had a second flaw: its date-bundling regex can match the `/` of `</em>` (e.g. `/<em>daily</em>/2024/01/02`), desyncing the chains and dropping every ancestor highlight. The fix splits the highlighted string on `/` outside tags and accepts a segment's markup only when its tags are balanced and its tag-stripped text equals the plain segment, so an `<em>` crossing a `/` falls back to plain text for just that segment instead of injecting `<em>B` / `C</em>` fragments.
  - **The opt-in page-name logic no longer touches the default path**: without the renderer, `PageListItemL` computes the page name exactly as before this spec (Requirement 8.1).
  - **`isPathTruncationEnabled` → `renderTruncatedAncestorPath` render prop** so `PageListItemL` no longer imports `features/search` (see research.md "Opt-in prop name and default").
  - **Root separator restored** (Requirement 2.2): `SearchResultAncestorPath` now always renders the `/` after the home/trash icon like `PagePathHierarchicalLink` (inside the home link; a separate link to `/` after the trash icon), including with zero ancestors.
  - **Shape guard removed**: `buildAncestorPathNodes` maps surviving parts to chain positions from the head (before the ellipsis) and the tail (after it), which works for any single-ellipsis shape, so the unreachable `hasExpectedShape` check and its `logger.error` (and the `loggerFactory` dependency) are gone.
