...
- **追補 (2026-06-15, フラグ全廃へ方針変更)**: 上記「見送り」を取り消し、enum 6 件 (server-reachable 5 + client `SyncExecution` 1) を const-object+union、parameter property 3 件 (nchan / sse-helper / llm-response-stream-processor) を明示フィールド代入に **erasable 化**。`--experimental-transform-types` を全 script (= `tsrun` 定義 1 箇所) から削除し、Node 24 デフォルトの **strip-only (フラグ無し)** で実行する。`tsconfig.json` に `erasableSyntaxOnly: true` を常設し、`tsgo --noEmit` (CI lint) が非 erasable 構文の再混入を検出するので、フラグ全廃が恒久維持される。懸念した型カスケードは `SearchDelegatorName.X` を**型位置**で使う 2 箇所のみ (`elasticsearch.ts` / `private-legacy-pages.ts`、`typeof` 追加で解消)。フラグ無し strip-only で crowi グラフ cold-load 成功 (3.40s) を実証。本番 `migrate:umzug` もフラグ不要に。`module.registerHooks` (resolver) は alias 解決に必須なため残るが、これは起動フラグではなく experimental API。
- **transform を採用しなかった理由 (一度は採用 → strip へ切替, 実行時性能の実測根拠)**: 同一 erasable コードでフラグ有無のみを変えた純粋比較 (crowi グラフ cold-load, warm disk, 各 4 回) で、**strip (フラグ無し) 中央値 ~1.54s に対し transform (`--experimental-transform-types`) 中央値 ~1.82s**。strip が約 15% (~0.3s) 速い。理由は strip が型削除のみ、transform は該当構文が無くても enum/namespace/parameter-property の変換パスを通すため。当初記録した「2.42–2.79s (transform) vs 3.40s (strip)」は別タイミング・別 disk-cache 状態での非同条件計測で直接比較できておらず (strip が遅く見えたのは cold-cache の単発ノイズ)、同条件で測り直すと strip ≤ transform が成立する (strip ⊆ transform の包含関係どおり)。**結論: transform を採用せず strip を選んだのは、experimental 起動フラグの排除 (機能・運用面) に加えて実行時オーバーヘッドの面でも有利だから**。transform 採用は enum/parameter-property をソース変更せず動かせる利点があったが、(1) experimental フラグが本番含め残る、(2) strip より僅かに遅い、の 2 点で erasable 化 + strip に劣ると判断した。
