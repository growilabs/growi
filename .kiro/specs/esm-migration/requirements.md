...
- **In scope**: ワークスペースルート・`apps/app`・残る CJS 共有パッケージへの `"type": "module"` 宣言付与、`apps/app` サーバビルドの ESM 出力化、サーバソースからの CJS 固有構文排除、`transpilePackages` の削減、サーバが CJS であったことのみを理由に存在する `pnpm.overrides` エントリの削除、消費側 CLI の都合で CJS のまま残す必要がある JavaScript ファイルの `.cjs` へのリネームまたはディレクトリ単位での明示的 CJS 化。**(2026-06-17 追加)** 外部プラグインの導入経路 (インストール → 静的配信 → ブラウザ読み込み / サーバ側走査) が ESM 移行後も機能することを確かめる smoke 検証。本移行は `features/growi-plugin/server` のルートファクトリ・ルーター登録・動的 import 変換に手を入れており (task 3.3.g 他)、これらは build / boot の検証では実行されないため、公式リリース済みプラグインを各種別 1 つずつ実際にインストールして確認する。
- **Out of scope**: `apps/slackbot-proxy` (廃止予定)、ESM 互換性のために必要な範囲を超える Crowi DI アーキテクチャの再設計、ESM 化によって初めて可能となる破壊的依存アップグレード (例: `@keycloak/keycloak-admin-client` メジャーバージョン更新)、`apps/app/src/migrations/*.js` の ESM 変換。**外部プラグインそのものの再ビルド・プラグイン開発側 API / `@growi/pluginkit` 公開フォーマットの変更・新しいプラグイン互換性保証の策定は対象外** — 本 spec が確かめるのは「既存の導入経路が ESM 移行で壊れていないこと」だけであり、プラグイン側のコードには一切手を入れない。
- **Adjacent expectations**: ESM-only 依存を推移的に取り込むサードパーティ CJS パッケージが実行時に解決されるよう、Node.js 24 の `require(esm)` が利用可能であり続けること。`transpilePackages` 削減後も残るパッケージに対しては Turbopack の解決・外部化が引き続き機能すること。`migrate-mongo` が明示的に CJS として扱われるディレクトリから CJS マイグレーションファイルをロードし続けること。外部プラグインはビルド済み `dist/` (Vite マニフェスト同梱) を伴って配布され、GROWI はそれを再ビルド・再リンクせずに静的配信 (script/theme) もしくはサーバ側走査 (template) するという配布の取り決めが維持されること。
...
7. When 移行全体が完了した場合, the 本番アーティファクトに対して公式リリース済みプラグインを各種別 (script / theme / template) 1 種類ずつ管理 API 経由でインストールする smoke shall 成功し、以下を確認する: (a) `POST /_api/v3/plugins` が成功を返し `growiplugins` ドキュメントが種別どおりに作成される、(b) script / theme は `retrieveAllPluginResourceEntries()` がビルド済み資産 (JS / CSS) を返し、`/static/plugins/...` で当該資産が配信される (HTTP 200)、(c) template はサーバ側でテンプレートが走査される。プラグイン側コードは一切ロード・ビルドしない。
8. Where Requirement 6.7 の smoke が失敗する場合, the 移行プロセス shall 失敗が外部プラグイン (プラグイン側のビルド欠落・ディレクティブ不備など) ではなく GROWI 側の導入経路の退行であることを切り分けたうえで是正する。
