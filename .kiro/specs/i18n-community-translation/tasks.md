...
- [ ] 5.5 非ソース言語の既存翻訳をPOEditorへ初回投入するツールを作る（6.2の実環境確認で発見した必要性への対応）
  - [x] `apps/app/tools/i18n-sync/seed-existing-translations.ts`（`pnpm run i18n:sync:seed`）を新設する。指定した非ソース言語1つについて、全namespaceの現在のリポジトリ内容を統合し、`syncTerms: false`・タグ無しの非破壊アップロードを1回行う（termの作成・削除やタグ付けはPushSourceSyncの責務のまま変更しない）
