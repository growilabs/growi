...
_残存ギャップ: 実際に GitHub の知識がない利用者が POEditor の参加導線・投稿画面を迷わず使えるかは、自動テストでは検証できない。_
...
_残存ギャップ: テストプロジェクトでの実測により正しく動作することは確認済みだが、POEditor 側の API 仕様が将来変わったこと自体は自動テストでは検知できない(design.md の Revalidation Triggers 参照)。_
...
_残存ギャップ: `master` の branch protection / Mergify のルール構成が変わった場合に、この自動反映の仕組みが引き続き成立するかは自動テストの対象外(design.md の Revalidation Triggers 参照)。_
...
_残存ギャップ: 「アプリケーション本体が実行時に POEditor を呼ばない」ことを継続的に保証する自動ドリフトテストは無く、将来の変更でこの境界が破られていないかはコードレビューに依存する。_
...
_残存ギャップ: 進捗表示は POEditor 自体の画面に依存しており、POEditor 側の UI 変更や利用制限は自動テストで検知できない。_
...
_残存ギャップ: `packages/editor` 側が将来新しい `toolbar.*` キーを追加したときに、同期設定への追加が漏れないかは自動テストでは検知できず、レビュー時の確認に依存する。_
...
_残存ギャップ: OSS プランの承認可否・継続可否は人手の手続きであり、自動テストの対象外。_
...
_残存ギャップ: 非ゼロ終了・ログ出力までは自動テストで検証済みだが、GitHub Actions の失敗通知が実際にメンテナーへ届くか(メール・Slack 等の通知経路)までは自動テストの対象外。_
...
_残存ギャップ: タグ付けアップロード自体は自動テストで検証済みだが、POEditor 画面上でのタグ絞り込みの実際の使い勝手は自動テストの対象外。_
...
_残存ギャップ: 現行5言語の対応関係は単体テストで検証済みだが、GROWI が新しい言語を追加したときに `LanguageCodeMap` への追加が漏れないかは自動テストでは検知できず、レビュー時の確認に依存する。_
