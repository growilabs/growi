...
背景・スコープ・依存関係の詳細は [brief.md](./brief.md)、アンブレラ全体は [roadmap.md](../../steering/roadmap.md) を参照。
...
- **Adjacent expectations**:
  - 基盤 `editor-slash-command` のスラッシュコマンド機構（起動・絞り込み・`apply`・i18n 解決）に乗る。基盤の起動条件（行頭のみ）に従う。
...
3. While 基盤の起動条件（行頭での `/`）を満たさない, the 拡張要素コマンド shall 候補として表示されない
