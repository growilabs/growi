...
この文書は drawio spec の調査記録で、**フェーズごとに追記した 4 つの節が時系列で並んでいる**。途中から読むと古い記述を現況と取り違えるので、まず下の表で当たりを付けること。

| 節 | いつ書いたか | 今の扱い |
|---|---|---|
| `Gap Analysis: drawio` | 要件を書いた直後 | **一部が古い。** とくに `2.1 テストの穴` の表は、リダイレクトの担保について誤っており、AC 番号も要件 3 の再採番前（節の冒頭に訂正注記あり） |
| `Design Discovery & Synthesis: drawio` | design を書くとき | 有効。設計判断の理由と却下した案が載っている |
| `README 移設の対応表（タスク 1.1）` | 実装（タスク 1.1） | 有効。README 147 行を消してよいと判断した根拠そのもの |
| `担保テストの突き合わせ（タスク 1.2）` | 実装（タスク 1.2） | **担保テストについてはこれが正。** 55 の受け入れ基準ごとの判定が載っている |

現況の設計・契約・検証手順は [design.md](design.md) が持つ。この文書は「どうしてそうなったか」の記録である。
