...
| 参照先の差し替え | `rebase-asset-paths.spec.ts` | query を落とすこと、サブパスの保持、繰り返し適用しても安全なこと |
| 2 つの入口 | `index.spec.ts` | 既定のとき・解釈できない値のときに何もしないこと、自前ホストで両方走ること |
| 自前ホスト判定 | `is-self-hosted-drawio.spec.ts` | 判定の基準、解釈できない値 |
| 配信の部品 | `drawio-assets.spec.ts` | 許可された形の判定、取得先の解決、範囲外の拒否、要求直前の範囲の再確認、バイト列がそのまま通ること、到達不能で投げないこと |
...
| 保存経路 | `DrawioCommunicationHelper.spec.ts` | 保存されること、内容が取れないときに上書きしないこと |
...
|---|---|---|
| 実際に数式が描画される | 1.1, 1.3, 1.4, 1.5 | 下記の手動確認 |
| 実際に図形が描画される | 2.1, 2.3, 2.6 | 下記の手動確認 |
| ブラウザから本家へ要求が出ない | 2.2 | 下記の手動確認 |
| 両方失敗したとき図形だけが空になる | 2.5 | 未検証（将来課題） |
| **配信ルータの応答**（状態コード・ヘッダ・外部要求の有無・フォールバック・既定構成での 404・認証を通らないこと） | 3.2〜3.10 | **ルータ本体を呼ぶテストが 1 件も無い**（将来課題）。現状は手動確認のみ |
| メニューの文字が実際に判読できる | 4.2 | 実測でコントラスト比 1.05 → 修正を確認。自動テストは構造のみ |
| 発信元が一致しないメッセージを処理しない | 6.5 | テスト無し（将来課題） |
| ページ送りが実際に保たれる | 7.1 | 下記の手動確認 |
...
| 課題 | 種類 | 由来 |
|---|---|---|
| 配信ルータの応答を検証するテストの追加 | テストの穴 | 要件 3 の担保が判定部品のみ |
