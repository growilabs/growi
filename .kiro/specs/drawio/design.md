...
- **担保が無い振る舞いへの自動テストの追加**（要件 11。2026-08-04 に境界を拡張）。挙動は変えない。
- **保守情報の入口** — `features/drawio/CLAUDE.md` の内容と、そこから spec への導線。

### Out of Boundary

- **挙動の変更**。設定不正時に利用者へ理由を伝えることは将来課題であってこの spec の作業ではない。**テストの追加は挙動を変えないので境界の内側**（要件 11）。ただしテストのために本番コードの形を変えることはしない（テスト用の縫い目を入れる必要が出たら、そこで止めて記録する）。
...
- Risks: 応答の状態コード・ヘッダ・外部要求の有無・フォールバック・502 はタスク 4.1 で担保した（[Testing Strategy](#testing-strategy)）。**残る穴は取得の時間の上限**で、テスト側から早送りできないため繰り越している（[将来課題](#将来課題)）。
...
| 配信ルータの応答 | `drawio-assets.spec.ts` | 許可外パスと既定構成で 404 かつ**外部要求 0**、範囲外で 404、成功時の Content-Type・`nosniff`・`Cache-Control`・バイト列、インスタンスが返せないときの本家への切り替え、両方失敗で 502、認証を通らずに到達できること。fixture が要求パスを記録するので「拒否した」と「外へ出てから 404 になった」を区別できる |
| 取得のサイズの上限 | `drawio-assets.spec.ts` | 上限を 1 バイト超える本体が拒否されること、および実在する最大のライブラリと同じ大きさ（7 MiB）が通ること。**上限を上げる変更でも下げる変更でも落ちる** |
| postMessage の全分岐 | `DrawioCommunicationHelper.spec.ts` | 発信元の照合・`configure`・`ready`（復元）・保存・空メッセージ（閉じる）・該当なしの 6 分岐すべて |
| 2 つの入口 | `index.spec.ts` | 読み込み前と**読み込み後**の両方。自前ホストで MathJax が設定済みインスタンスへ移ること、既定構成と解釈できない値で何もしないこと、2 つを `DrawioViewerScript` と同じ順で走らせた結果 |
| 判定の単一性 | `no-duplicate-self-hosted-judgement.spec.ts` | draw.io 本家のオリジンへの言及が許可した 2 ファイル以外に現れないこと。走査が空になって無条件で通る状態も防いである |
...
| 両方失敗したとき図形だけが空になり、ページの描画は続く | 2.5 | 下記の手動確認。サーバ側が 502 を返すことは担保あり（ルータのテスト）。ビューアがその 502 を受けてページの描画を続けることは未検証 |
| 取得の**時間**の上限（10 秒） | 3.7 | テスト側から上限を早送りできず、実時間で 10 秒待つテストしか書けないため繰り越した（[将来課題](#将来課題)に実測と判断）。**サイズの上限（64 MiB）は担保あり**（上下の両側から挟む 2 件。要件 3 の AC 7 の担保欄） |
| メニューの文字が実際に判読できる | 4.2 | 実測でコントラスト比 1.05 → 修正を確認。自動テストは構造のみ |
| 保存した図がエディタで**実際に**全ページ復元される | 6.2 | 下記の手動確認。GROWI 側が保存内容に手を加えず返すことは担保あり（`ready` 分岐のテスト）。受け取った draw.io が全ページを描くことはブラウザでしか確かめられない |
| ページ送りが実際に保たれる | 7.1 | 下記の手動確認。1 ページ目に戻される原因を防ぐ判定と、ページ送りが出ること自体は担保あり |
| 既定構成が従来どおり | 8.2 | 下記の手動確認 |

### 追加する自動テスト（要件 11）

要件 1〜8 の担保欄で「担保が無い」と判定した箇所のうち、自動テストで捕まえられるものを足した（タスク 4.1〜4.5、実装済み）。**本番コードは 1 行も変えていない。** 時間の上限だけは繰り越した（[将来課題](#将来課題)に実測と判断）。

| 追加先 | 何を検証するか | 対応する要件 |
|---|---|---|
| `server/routes/drawio-assets.spec.ts`（追記） | ルータの応答 — 許可外パスと既定構成で 404 かつ外部要求 0、範囲外で 404、成功時の Content-Type・`nosniff`・`Cache-Control`・バイト列、インスタンスが返せないときの本家への切り替え、両方失敗で 502、認証を通らずに到達できること | 11.1〜11.5、2.4、3.2〜3.5、3.8〜3.10 |
| `server/routes/drawio-assets.spec.ts`（追記） | 取得の時間・サイズの上限 | 11.6、3.7 |
| `DrawioModal/DrawioCommunicationHelper.spec.ts`（追記） | 未テストだった 4 分岐 — 発信元の不一致、`configure`、`ready`（復元経路）、空メッセージ（閉じる） | 11.7、6.2、6.5 |
| `client/self-hosted/index.spec.ts`（追記） | 読み込み後の入口が自前ホストのときだけ効くこと | 11.8、8.1、8.3 |
| `features/drawio/` 配下に新規の drift spec | 自前ホスト判定が単一の関数に集約され続けること | 11.9、8.4 |

**ルータのテストの組み立て方**: `express()` に `drawioAssetsRouterFactory()` を素で mount し、`supertest` で叩く。`~/server/service/config-manager` と `~/utils/logger` は `vi.mock` で差し替える（前例: `features/growi-vault/server/routes/__tests__/vault-admin-reconcile.spec.ts`）。上流は既存の spec と同じく `node:http` の fixture サーバを立て、**要求が来たかどうかを記録して「外部要求 0」を検証できるようにする**（「404 が返った」だけでは、外に出てから 404 になった場合と区別できない）。

**判定の drift テストについて**: 「判定が 1 か所にある」ことを、その関数の呼び出し箇所を数えて確かめても意味がない（別実装を足しても呼び出し箇所は減らない）。守りたいのは「**自前ホストかどうかを決めるコードが他に生えていないこと**」なので、`features/drawio/` 配下のソースを走査して、既定オリジンとの比較が `consts.ts` と `is-self-hosted-drawio.ts` 以外に現れないことを検証する。essential-test-design の Guard / Drift Specs に従い、**意図的に違反を入れて RED を確認してから**加える。
...
| 課題 | 種類 | 由来 |
|---|---|---|
| 取得の**時間**の上限（10 秒）のテストの追加 | テストの穴 | 要件 3.7。サイズの上限（64 MiB）は担保済みで、時間の側だけが残っている。**繰り越した理由（実測）**: ① vitest の fake timer は `AbortSignal.timeout` を動かせない。上限の 10 倍まで `vi.advanceTimersByTimeAsync` で進めても `signal.aborted` は `false` のままだった。Node はこの打ち切りを、テスト側から差し替えられない場所のタイマーで予約している（`globalThis.setTimeout` は使わない。素の node スクリプトでは `node:timers` の `setTimeout` を経由するのが確認できるが、vitest の中ではそこを橋渡ししても呼ばれない）。② 残る手段は実時間で 10 秒待つテストだけで、実測 10,006 ms。この spec ファイルの実行時間が 0.47 秒から約 10.5 秒になり、vitest の既定の 5 秒制限を超えるので個別の時間指定も要る（その値自体が上限の値を書き写すことになる）。③ `AbortSignal.timeout` の呼び出しを spy する形は、上限が効くことではなく上限をどう実装したかを見るテストになる（essential-test-design の Anti-Pattern 1）。応答が速い fixture を当てる形は上限について何も示さない。**上限は暴走の歯止めであって、そのために毎回 10 秒払う取引はしない**（要件 11.6）。本番コードに上限を注入できる縫い目を入れれば書けるが、それは本番コードの形を変えることなので対象外 |
| 両方失敗時に**ビューア側で**図形だけが空になることの検証 | 未実施の検証 | 要件 2.5。サーバが 502 を返すことはタスク 4.1 で担保した。ビューアがそれを受けてページの描画を続けることはブラウザでしか確かめられない |
