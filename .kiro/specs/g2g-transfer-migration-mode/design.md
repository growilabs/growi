...
- **D5: 取り込み中の保護は既存の保守モードで行い、設定の取り込みの後に DB へ旗を書き戻す。** 判定の仕組み（`isMaintenanceMode()` と middleware）は変更しない。新しい概念も新しいモジュールも作らない。
  ...
  - **書き戻す値は `true` 固定ではなく「設定の取り込みに入る直前の値」**。この処理は取り込みの共通処理にあるので、G2G の引っ越し・従来モード・管理画面からの zip 取り込みのすべてを通る。`true` 固定にすると、保守モードでない転送（従来モードの既定）でも旗が立ち、しかも受信側は保護を起こす条件を満たしていないので後始末を走らせず、**旗を下ろす者が誰も居ないまま移行先が保守モードで固まる**（要件 6.1 違反）。読む先はメモリで足りる（設定コレクションを空にしてもメモリの値は落ちないことが上記の前提）。管理画面からの zip 取り込みは保守モードを必須にしているので（`routes/apiv3/import.ts:284-289`）、取り込み前の値は必ず `true` で、同じ規則で正しく動く。
...
| 書く（書き戻す） | 取り込みの共通処理 | 設定の取り込みの後（成否に関わらず） | **取り込み前の値**を書き戻す。`true` 固定にしない |
...
- **書き戻す値は「設定の取り込みに入る直前の値」で、`true` 固定にしてはならない**。この処理は G2G の両モードと管理画面からの zip 取り込みのすべてを通る。`true` 固定にすると保守モードでない転送でも旗が立ち、受信側の後始末も走らないので移行先が保守モードで固まる（要件 6.1）。
...
- **従来モードの転送の後、移行先が保守モードになっていない**（書き戻しが取り込み前の値を使っていることの確認）（6.1, 2.4）。
- **転送前から保守モードだった移行先が、転送の後も保守モードのまま**（後始末が解除ではなく復元であることの確認）。引っ越しと従来モードの両方で確かめる（6.1, 2.8）。
