...
- [x] 2.7 Clear passwordHash in statusDelete
  - Add `this.passwordHash = undefined;` to `statusDelete()` (`apps/app/src/server/models/user/index.js` v8: line ~371, with `this.password = ''` at line ~379) so that deleted users do not retain a valid scrypt credential hash (same intent as the existing `this.password = ''`)
