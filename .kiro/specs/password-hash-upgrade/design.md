...
**Rollback**: Before Phase 3 (Cleanup), the `password` field is retained, so an immediate recovery via a code rollback is possible. After Phase 3, reset emails need to be sent with the Downgrade prep script.
