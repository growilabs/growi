...
- **Remaining exposure (accepted, not closed here)**: the limiter is keyed per IP (and per user), so a *distributed* credential-stuffing campaign still spends one scrypt per source IP and can saturate the libuv pool — there is no global concurrency cap on password verification. The keys corrected here are the four auth endpoints above, plus `/installer` (the scrypt-running POST is `/_api/v3/installer`; the bare key is kept as a separate GET entry for the legacy installer page, because `usersPerIpProspection` applies regardless of method) and `/_api/check_username` → `/_api/v3/check-username`. **This is not a complete audit of the rate-limiter config, and was not meant to be** — the sweep started from the login key this spec's scrypt change made load-bearing and stopped at the endpoints that hash a password. Keys for other endpoints may be stale in the same way; one known case is `POST/PUT /_api/v3/forgot-password`, which is unauthenticated and whose PUT runs `updatePassword` → scrypt, while the existing `/forgot-password/.*` regex key is anchored (`^`) and therefore covers only the page routes mounted at the app root. Auditing the remaining config belongs to a separate rate-limiter task, not to this spec
