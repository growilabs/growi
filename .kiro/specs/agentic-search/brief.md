...
- `apps/app/src/features/mastra/server/services/mastra-modules/tools/` 配下に **`getPageContentTool`** を新設。
  ...
  - 処理: `requestContext` から `userId` を取り出し、`Page.findByIdAndViewer` または `Page.findByPathAndViewer` を呼び出す。
  - 出力: `{ path, revision body (markdown), updatedAt, tags?, grant }` 程度。閲覧不能時は明示的にその旨を返す（agent 側で「権限がないので参照できません」と回答できる）。
