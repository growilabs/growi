...
## Adjacent Spec Assumptions

本 spec の agent ループ（R1.1〜1.3）は隣接する **既存 ES 全文検索 tool**（ユーザー作業ブランチで先行作成）に依存する。本 spec が成立するためには、その tool が以下の最小契約を満たしている必要がある。

### 全文検索 tool の最小出力契約

- **必須**: 出力に `pagePath: string` を含むこと（`getPageContentTool` の `pagePath` 入力に直接渡せる形）
- **推奨**: `pageId: string` も併記される場合は ObjectId 文字列形式であること（`pageId` 経由の本文取得を可能にする）
- **禁止**: ページ本文（フル markdown `body`）を返さないこと — 本文取得は本 spec の責務として `getPageContentTool` に集約する。全文検索 tool が本文を返すと役割重複と grant 二重実装の温床になる
- **推奨**: ハイライト断片 (`snippet`) を返すこと（agent が「本文取得が必要か」を判断する材料）
- **権限**: 既存 ES service の `filterPagesByViewer()` 経由で grant が反映済みであること（本 spec はこの前提を信頼し、`getPageContentTool` 側でも再度 grant 委譲を行う多層防御）

### 乖離検知時の責務分担

- フィールド名 / 型の乖離（例: `pagePath` ではなく `path`、`pageId` ではなく `id`）が見つかった場合: **本 spec の `getPageContentTool` 入力スキーマで吸収しない**。`growiAgent.instructions` でフィールドマッピングを LLM に任せるか、全文検索 tool 側 spec で命名を揃える。設計上の権威は全文検索 tool 側の spec とする
- 全文検索 tool が本文を返す変更が入った場合: 本 spec のコアな存在意義が揺らぐため、design 再生成または spec マージで対応する
- マージ順序: 本 spec は全文検索 tool のマージ後にマージするのが望ましい。未マージ状態で本 spec のみマージされた場合、agent は本文取得 tool のみで動作（部分機能）し、要件 1.1 は満たさない
