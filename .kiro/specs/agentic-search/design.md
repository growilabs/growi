...
- `growiAgent.tools` の構成変更（新 tool 2 つ登録 + `fileSearchTool` のコメントアウト）と `growiAgent.instructions` の文言調整
...
| File | 変更内容 |
|---|---|
| `agents/growi-agent.ts` | `fileSearchTool` の import + `tools` 登録をコメントアウト。`fullTextSearchTool` / `getPageContentTool` の import 追加と `tools` 登録（`fullTextSearchTool` は `crowi.searchService.isElasticsearchEnabled` で条件分岐）。`instructions` に「全文検索結果のうち根拠が必要なものは本文取得 tool を呼んで引用パスを含めよ」と「`fullTextSearch` の `query` は `"..."` / `-word` / `prefix:/path` / `tag:foo` 等の演算子を組み合わせ可」を英語短文で追記 |
| `routes/post-message.ts` | **モジュールスコープの `const requestContext = new RequestContext<...>()` を削除し、ハンドラ関数内で `new RequestContext<{ vectorStoreId: string; userId: string }>()` を生成する構造に変更**（並列リクエスト干渉防止、Issue 1 対策）。`requestContext.set('vectorStoreId', ...)` の直後に `requestContext.set('userId', req.user._id.toString())` を追加 |
...
sequenceDiagram
    ...
    PostMessage->>PostMessage: new RequestContext + set vectorStoreId + userId
    PostMessage->>Agent: stream messages requestContext
    Agent->>FullText: tool call query
    FullText->>FullText: read userId from requestContext
...
|---|---|---|---|---|---|
| `FullTextSearchTool` | Tool | 自然言語クエリで wiki 検索ヒット（pagePath / pageId / snippet）を grant 委譲取得。ES 未設定環境では agent に登録されない | 6.1–6.8, 3.2 | `SearchService.searchKeyword` (P0), `SearchService.isElasticsearchEnabled` (P0), `RequestContext` (P0) | Service |
...
- Outbound: `SearchService.searchKeyword()` — grant 委譲経路（P0）
...
- **Preconditions**: `requestContext` に `userId` がセットされている。SearchService が初期化済みでクエリ可能な状態
...
**Responsibilities & Constraints**
- `tools` に `fullTextSearchTool` と `getPageContentTool` を登録
- **`fullTextSearchTool` の登録は `crowi.searchService.isElasticsearchEnabled` を見て条件分岐**。ES URI 未設定の OSS デプロイでは `fullTextSearchTool` を `tools` に含めず、agent から見えなくする（実装は `agents/growi-agent.ts` のモジュール初期化時に判定）。`getPageContentTool` はこの条件に依存せず常に登録
- `fileSearchTool` の import 行と tools 登録行をコメントアウト（コードは残置）
- `instructions` に以下の趣旨を英語短文で追記（既存トーン維持、合計 3〜4 行程度に収める）:
  - 「wiki 内コンテンツ関連の質問はまず `fullTextSearch` tool を呼び、必要に応じて `getPageContent` tool で本文を取得して引用パスを回答に含めること」
  - 「`fullTextSearch` の `query` には自然言語に加えて以下の演算子を必要に応じて組み合わせて良い: `"phrase"`, `-term`, `prefix:/path`, `tag:foo`, `-prefix:` / `-tag:`（全て AND）。これらは subtree / タグ絞り込み・ノイズ除去のために使う」
...
export const growiAgent = new Agent({
  id: 'growiAgent',
  name: 'GROWI Agent',
  instructions: /* 既存指示 + 「fullTextSearch → 必要なら getPageContent → 引用パスを回答に含める」+ 「query は自然言語 + "..." / -word / prefix:/path / tag:foo の演算子を必要に応じて組み合わせ可」 */,
  model: getOpenaiProvider()(model),
  tools: {
    // fileSearchTool, // disabled: see spec agentic-search
    ...(crowi.searchService.isElasticsearchEnabled ? { fullTextSearchTool } : {}),
    getPageContentTool,
  },
  memory,
});
...
**Implementation Notes**
- Integration: `fullTextSearchTool` と `getPageContentTool` の両方を `tools` に登録。両 tool は本 spec で同時に新設するためマージ順序問題なし
- Validation: build + lint で「未使用 import の指摘が出ないこと」を確認（コメントアウトされた `fileSearchTool` の import が lint で警告にならない書き方を選ぶ）
- Risks: instructions の文言が冗長になると LLM のコンテキスト消費が増える。1〜2 行の追記に留める
...
- **`RequestContext` インスタンスをハンドラ関数内で `new` する**（モジュールスコープ singleton を廃止）。これにより並列リクエスト下で他リクエストの `userId` が tool に渡る可能性を排除
...
3. `SearchService.searchKeyword` をモックして結果配列を `{ pageId, pagePath, snippet }` 形にマップ（6.2, 6.3, 6.4）
4. SearchService の戻り値に `body` が含まれていても tool 出力には含めないこと（6.5）
5. SearchService が reject された場合に `result: 'error'` を返し execute が throw しないこと（6.8）
6. `userId` が ObjectId 文字列として SearchService に渡されること（6.7）
7. **クエリ構文の素通し**: `query` に `prefix:/docs -draft tag:meeting "release notes"` 等の演算子を含む文字列を渡したとき、tool 層で文字列が改変されず `SearchService.searchKeyword` の第 1 引数にそのまま渡ること（サニタイザ不在の保証、本 spec の Plan A 採用根拠）
...
- **`requestContext` のリクエスト隔離（本 spec で対処）**: `userId` は認証ミドルウェア通過後の `req.user._id` から派生するため改竄の余地なし。`@mastra/core` の `RequestContext` は内部的に単純な `Map` ラッパーで自動的なリクエスト隔離機構を持たないため、本 spec ではモジュールスコープ singleton を廃止しハンドラ関数内で `new RequestContext()` する。この変更により、並列リクエスト下で他リクエストの `userId` が tool 内 `get('userId')` で読み出される可能性を排除する
