# Gap Analysis: ai-provider-settings
...
`apps/app/src/server/service/config-manager/config-definition.ts` に下記キーが定義済み。

> 注(ai-provider-multi-model 反映後): 当初は `ai:model`(単一)/ `ai:providerOptions`(グローバル単一)を含む 5 キーだったが、ai-provider-multi-model により両者は `ai:allowedModels`(各エントリに modelId + 任意 providerOptions + isDefault)へ統合・廃止され(env `AI_MODEL` / `AI_PROVIDER_OPTIONS` も廃止・自動移行なし)、現在は下記 4 キー。

| Config Key | 環境変数 | 型 | isSecret |
|---|---|---|---|
| `ai:provider` | `AI_PROVIDER` | `AiProvider \| undefined` | - |
| `ai:apiKey` | `AI_API_KEY` | `string \| undefined` | ✅ |
| `ai:allowedModels` | `AI_ALLOWED_MODELS` | `AllowedModel[] \| undefined`(`{ modelId; providerOptions?; isDefault? }[]`、JSON 配列、`defaultValue: []`) | - |
| `ai:azureOpenaiSettings` | `AI_AZURE_OPENAI_SETTINGS` | `AzureOpenaiConfig`(`{ resourceName?, baseURL?, apiVersion?, useEntraId? }`、JSON オブジェクト、`defaultValue: {}`) | - |
...
- プロバイダー定義: `apps/app/src/features/mastra/interfaces/ai-provider.ts` — `AI_PROVIDERS`(openai, anthropic, google, azure-openai)+ `isAiProvider()` 型ガード（現行は provider ごとの `enumerable` メタデータマップ `AI_PROVIDER_DEFS` を源泉とし、`AI_PROVIDERS`/`AiProvider` はそこから導出＝ai-provider-model-picker。メンバー・ガード挙動は同じ）
- 消費側: `apps/app/src/features/mastra/server/services/ai-sdk-modules/resolve-mastra-model.ts` — **モジュールスコープでメモ化**(無効化機構なし)。設定不備時は throw(throw 時はメモ化されない)。注(ai-provider-multi-model 反映後): `resolveMastraModel(modelId?)` がモデル引数を受け取り、メモは `${provider}:${effective}` をキーとする Map に変更された
- provider オプションの解決: `resolve-provider-options.ts` — パース失敗時は警告ログ + `{}` フォールバック(fail-soft)。注(ai-provider-multi-model 反映後): グローバル単一 `ai:providerOptions` ではなく `getProviderOptionsForModel(effectiveModelId)` が**実効モデル ID を引数に取り**、その `ai:allowedModels` エントリの `providerOptions` を返す純粋なルックアップ（一律適用は廃止、自前のラウンディング/検証は行わない）。実効モデルの解決は呼び出し側(post-message ハンドラ)が `resolveEffectiveModelId` で 1 度だけ行い、その ID を requestContext と本関数の両方へ渡す（実効モデルをリクエストあたり 2 度解決しない）
...
4. **Research Needed(design で解決)**:
   ...
   - provider オプションのサーバー側バリデーション仕様(JSON 形式チェックのみ / 形状 `ModelProviderOptions`(= `Record<string, Record<string, JSONValue>>`、`interfaces/allowed-model.ts` 宣言)まで検証するか)。注(ai-provider-multi-model 反映後): グローバル単一 `ai:providerOptions` ではなく `ai:allowedModels` の各エントリの `providerOptions` を `isValidProviderOptionsJson` で検証する
...
- `resolve-mastra-model.ts` の **モジュールスコープのメモ** のみ。早期 return するため、`configManager.loadConfigs()` で config 値が更新されても古いモデルを返し続ける(= 中核ギャップ)。注(ai-provider-multi-model 反映後): メモは `${provider}:${effective}` キーの Map になり、`clearResolvedMastraModelCache()` は Map 全消去
- `getProviderOptionsForModel()`(`resolve-provider-options.ts`)は**メモ化していない**(毎回 config を読む)ため、対応不要。注(ai-provider-multi-model 反映後): 引数に**実効モデル ID**(`effectiveModelId: string`、解決済み)を取り、その `ai:allowedModels` エントリの `providerOptions` を返す純粋なルックアップ(自前の解決/ラウンディングは行わない)
...
- **env 設定有無の検出**: `loadFromEnv()`(`config-loader.ts` L12-37)は env 未設定時に **default 値**を `envConfig[key].value` に格納するため、`envConfig[key].value` では「env が設定されたか」を判定できない(特に `ai:azureOpenaiSettings` は default `{}`)。信頼できる判定は **`process.env[envVarName] !== undefined`**(`getManagedEnvVars` L189-216 と同じ手法)
...
### 設計判断
- `isAiConfigured()`(provider + provider 別必須項目の**非 throw** 検査)+ `isAiReady() = isAiEnabled() && isAiConfigured()` を `features/mastra/server/services/is-ai-configured.ts` に新設。注(ai-provider-multi-model 反映後): 判定基準は **provider + provider 別の必須接続設定 + 非空 `ai:allowedModels`** に更新(モデル集合が空なら未構成)。azure-openai プロバイダーでは認証情報に加え**エンドポイント(`resourceName` または `baseURL`)も必須**(`resolveAzureOpenaiModel` がエンドポイント無しで throw するのに合わせる)。Azure OpenAI + Entra ID(`useEntraId === true`)のときだけ apiKey 不要、それ以外(非 Azure / Entra 無効の Azure)は共通 apiKey が必須。
...
- `app:aiEnabled` を `env:useOnlyEnvVars:ai` グループの対象キーに追加。env 専用モード時はトグルも env 固定・編集不可。注(ai-provider-multi-model 反映後): グループの対象キーは `app:aiEnabled` + `ai:provider` / `ai:apiKey` / `ai:allowedModels` / `ai:azureOpenaiSettings` の計 5 キー。
- **境界**: `isAiConfigured` の判定基準は `resolveMastraModel` の認証分岐(`requiresApiKey`)と一致させる(乖離すると「ready だが resolve で throw」が再発)。openai 側 certify-ai-service は変更しない。
