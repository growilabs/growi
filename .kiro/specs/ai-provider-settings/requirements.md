...
```
  ...
  // 旧 ai:model(単一)/ ai:providerOptions(グローバル単一)は ai-provider-multi-model
...
1. The AI 設定管理画面 shall `ai:provider`, `ai:apiKey` の各設定項目に対応する入力欄、および `ai:allowedModels`(許可モデルの集合)を編集するリストエディタを提供する。リストエディタの各行はモデル ID(Azure OpenAI ではデプロイメント名)・既定指定(リスト内ちょうど 1 つ)・モデルごとの provider オプション(任意の JSON)から成る。ai-provider-multi-model に整合し、旧 `ai:model`(単一)/ `ai:providerOptions`(グローバル単一テキストエリア)の入力欄は廃止
