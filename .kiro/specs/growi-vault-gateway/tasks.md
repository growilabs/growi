...
`/vault.git` ルータが GROWI 標準 middleware チェーンを通さず認可を自前実装していたため、(1) `restrictGuestMode='Deny'`（デフォルト）でも匿名 clone が通る ACL すり抜け（要件 2.4a）、(2) reverse proxy の Basic 認証と PAT が単一 `Authorization` ヘッダーで衝突（要件 2.6）、(3) 同一 feature 内の `vault-admin` / `vault-page` ルータとの不整合（要件 11）が生じていた。本タスクは gateway の認証・認可を標準チェーンへ合成し直す。**タスク 4・10 の認証部分を supersede する。**

> **依存・順序（重要）**: 要件 2.6 の実装（26.3）は PR #11244 の `extractAccessToken` / `X_GROWI_ACCESS_TOKEN_HEADER_NAME`（`apps/app/src/server/middlewares/access-token-parser/extract-access-token.ts`）に依存する。これは **master にのみ存在**し、現ブランチ `feat/growi-vault` には **master → dev/8.0.x → feat/growi-vault** のマージで到達する。26.3 はその helper が現ブランチに入った後に着手する。26.1 / 26.2 は既存の `aclService` / `loginRequiredFactory` のみで先行実装できる。
>
...
- **RED**: `vault-gateway.spec.ts`（必要なら `vault-pat-auth.spec.ts`）に次を先に追加し失敗を確認する:
  ...
  - `Authorization` ヘッダ無し かつ `isGuestAllowedToRead()=true`（`wikiMode='public'` または `restrictGuestMode='Readonly'`）→ 匿名で `['public']` のみ通過
- **GREEN**: `userId == null` のとき `aclService.isGuestAllowedToRead()` を参照。`false` なら 401（既存の認証失敗と同じ git fallback 経路）。`aclService` は singleton を直接 import（`~/server/service/acl`）
...
- `vault-admin` / `vault-page` と同様に crowi 注入で `loginRequired` を構築する（DI 維持、テストでは crowi 省略可）
- **意図的逸脱を明示**（要件 11.4 — 暗黙差分にしない）:
  - CSRF / `certifyOrigin` は適用しない（git クライアントは origin/CSRF を持たず read 系のみ）— コードコメント + テストで意図を記録
  - `excludeReadOnlyUser` は適用しない（read-only clone は許可）— 同上
...
- **RED**: `vault-pat-auth.spec.ts` に次を先に追加し失敗を確認する:
...
- `git config http.<url>.extraHeader "X-GROWI-ACCESS-TOKEN: <PAT>"` を設定すると `Authorization` を proxy の Basic 認証に空けたまま PAT を渡せること
- proxy 無しの通常ケースは従来どおり `https://x:<PAT>@host/vault.git` でよいこと
- **完了確認**: ドキュメントに proxy 配下 / 非配下 双方の手順が記載されていること
...
2. `/kiro-spec-requirements growi-vault-gateway` を再度実行し、新規 Req として「user-triggered targeted reconcile」を追加（注: Req 11 は本リビジョンで「標準 middleware 構成との整合」に割当済み。reconcile は別 spec `growi-vault-reconcile` で実装済みのため本 Planned Extensions は概ね解消済み）
