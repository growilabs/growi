...
12. （名前の長さの上限）エントリ名が UTF-8 で 255 バイトを超えるとき、VaultViewComposer は拡張子より前の部分を文字の境界で切り詰め、10 と同じ `__<hash8>` suffix を付与して 255 バイト以内に収める。判定は suffix を付けたあとの最終的な名前に対して行う（10 の suffix を付けたことで上限を超える場合も切り詰めの対象になる）。255 バイト以内に収まる名前は一切変更しない
