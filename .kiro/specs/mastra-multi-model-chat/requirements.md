...
2. **実行時にベンダー API からモデル一覧を取得する仕組みは設けない。** 理由: Vercel AI SDK (`ai`/`@ai-sdk/*`) は provider が factory のみでモデル列挙不可。唯一 `@ai-sdk/gateway.getAvailableModels()` はあるが Vercel Gateway 認証 (`AI_GATEWAY_API_KEY`/OIDC) 必須で、provider+apiKey を直接設定する GROWI に不適合（この実行時ルーター系の論拠は現在も有効）。（更新: ai-settings-model-picker により、カタログを持つプロバイダ (openai / anthropic / google) の管理者許可リストは、models.dev をリリース前段で取り込んだオフライン同梱カタログ (`model-catalog-data.json`、chat+tool 対応にフィルタ済み) からの**選択方式**を採用する。一覧提供（read パス）は成果物 read のみで外部通信なし（既定。opt-in のカタログリフレッシュ〔管理画面手動／起動時／定期〕の実行時に限り models.dev へ fetch する）。azure-openai・取得失敗時は従来どおり自由入力。詳細は `.kiro/specs/ai-settings-model-picker`。）
...
- **Out of scope (本機能は担わない)**:
  ...
  - 実行時にプロバイダやレジストリの API からモデル一覧を自動取得・補完すること（実行時ベンダー API 呼び出しは引き続きスコープ外）。（更新: ai-settings-model-picker により、カタログを持つプロバイダの許可リストはオフライン同梱カタログ (`model-catalog-data.json`) からの選択方式を採用。一覧提供（read パス）は成果物 read のみで外部通信なし（既定。opt-in のカタログリフレッシュ〔管理画面手動／起動時／定期〕の実行時に限り models.dev へ fetch する）。azure-openai・取得失敗時は自由入力。詳細は `.kiro/specs/ai-settings-model-picker`。）
