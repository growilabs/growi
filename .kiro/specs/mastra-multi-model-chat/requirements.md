...
2. **実行時にベンダー API からモデル一覧を取得する仕組みは設けない。** 理由: Vercel AI SDK (`ai`/`@ai-sdk/*`) は provider が factory のみでモデル列挙不可。唯一 `@ai-sdk/gateway.getAvailableModels()` はあるが Vercel Gateway 認証 (`AI_GATEWAY_API_KEY`/OIDC) 必須で、provider+apiKey を直接設定する GROWI に不適合（この実行時ルーター系の論拠は現在も有効）。（更新: ai-settings-model-picker により、カタログを持つプロバイダ (openai / anthropic / google) の管理者許可リストは、models.dev をリリース前段で取り込んだオフライン同梱カタログ (`model-catalog-data.json`、chat+tool 対応にフィルタ済み) からの**選択方式**を採用する。一覧提供（read パス）は成果物 read のみで外部通信なし（既定。カタログリフレッシュ〔管理画面手動／起動時／定期。起動時・定期は AI 機能有効時のみ作動し定期は既定日次〕の実行時に限り models.dev へ fetch する）。azure-openai・取得失敗時は従来どおり自由入力。詳細は `.kiro/specs/ai-settings-model-picker`。）
3. **config:** 新キー `ai:allowedModels: Array<{ modelId: string; providerOptions?: ModelProviderOptions; isDefault?: boolean }>` — モデル + provider オプション + 既定フラグを 1 エントリに同梱（既定は別キーではなくリスト内 `isDefault`、1 つ）。providerOptions の型は `ModelProviderOptions`（`Record<string, Record<string, JSONValue>>`、`interfaces/allowed-model.ts` に宣言。旧 `MastraProviderOptions` 型は削除）で、クロスレイヤ DTO とサーバ側 resolver が共通参照する。**`ai:model` と `ai:providerOptions` は完全廃止**（config キー・env `AI_MODEL` / `AI_PROVIDER_OPTIONS` ともに削除。自動移行なし）。グローバル既定オプションは設けない。env-only グループ `env:useOnlyEnvVars:ai` の `targetKeys` に `ai:allowedModels` を追加（旧 2 キーは除去）。providerOptions 検証は既存 `isProviderNamespacedObject` / `isValidProviderOptionsJson` を各エントリに再利用。`isAiConfigured()` は `provider が有効 + プロバイダ必須の接続設定 + 非空 allowedModels` に更新（azure-openai では apiKey/Entra ID 認証に加え endpoint（resourceName または baseURL）も必須。これは endpoint なしで throw する resolveAzureOpenaiModel と整合させるため。apiKey は Entra ID 利用時のみ免除）。
...
6. **リクエスト単位のモデル切替:** Mastra@1.41 の動的モデル関数を使う。`growi-agent.ts` を `model: ({ requestContext }) => resolveMastraModel(requestContext.get('modelId'))` に。`resolveMastraModel` を `(modelId?)` 対応 + Map 化（in-flight の Promise を保持する single-flight。同時ミスの二重構築と、ビルド中の設定保存による stale キャッシュを防ぐ）。providerOptions は `getProviderOptionsForModel(effectiveModelId)`（解決済みの実効モデル ID を受け取る pure な lookup。自前の検証/丸めはしない）で取得。post-message ハンドラはリクエストごとに `resolveEffectiveModelId` で実効モデルを **一度だけ** 解決し、その ID を requestContext と `getProviderOptionsForModel` の **両方** に渡す（実効モデルを 1 リクエスト内で二重解決しない）。各 provider resolver は modelId 文字列を引数受け取りに変更（`modelResolvers: Record<AiProvider, (modelId: string) => Promise<MastraModelConfig>>`。実装後の最適化で各 resolver は自分の `@ai-sdk/*` を関数内 `await import()` で遅延ロードするため戻り値は Promise。未使用 provider の SDK は読み込まない）。
...
8. **チャット UI:** ベンダリング済み `PromptInputModelSelect*` を ChatSidebar に mount。command-palette 型 `ModelSelector` は CSP 問題のため不採用。ユーザーの選択モデルは `UserUISettings`(DB) の `aiChatSelectedModelId` に永続化。**読取は `/mastra/models` がサーバ検証して `selectedModelId` を返す、書込は共有 `scheduleToPut`、チャットは feature ローカル useState**（専用 atom・SSR ハイドレートは持たない）。`/mastra/models` のレスポンスは共通インタフェース `ChatModelsResponse`（`interfaces/chat-models-response.ts` に宣言、ルートとクライアント SWR フックが共有）= `{ modelIds: string[]; selectedModelId: string }`。`modelIds` は許可モデル ID の素の配列（`{id,name}` や表示名は持たない）、`selectedModelId` は常に存在（AI 設定済み＝非空 allowed-list のときだけルートが動くためデフォルトが必ず解決でき、`defaultModelId` はレスポンスに含めない）。初期選択は「前回選択 ?? デフォルト」をサーバ一元で解決。
...
- **Out of scope (本機能は担わない)**:
  ...
  - 実行時にプロバイダやレジストリの API からモデル一覧を自動取得・補完すること（実行時ベンダー API 呼び出しは引き続きスコープ外）。（更新: ai-settings-model-picker により、カタログを持つプロバイダの許可リストはオフライン同梱カタログ (`model-catalog-data.json`) からの選択方式を採用。一覧提供（read パス）は成果物 read のみで外部通信なし（既定。カタログリフレッシュ〔管理画面手動／起動時／定期。起動時・定期は AI 機能有効時のみ作動し定期は既定日次〕の実行時に限り models.dev へ fetch する）。azure-openai・取得失敗時は自由入力。詳細は `.kiro/specs/ai-settings-model-picker`。）
