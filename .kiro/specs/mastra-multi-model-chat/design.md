...
**Impact**: 単一の `ai:model`（文字列）+ `ai:providerOptions`（単一 JSON、全 stream に一律適用）を、`ai:allowedModels`（モデル + providerOptions + 既定フラグを同梱した配列）+ リクエスト単位のモデル解決へ置き換える。既定モデルは別キーではなく**配列エントリの `isDefault` フラグ**で表す。`ai:model` / `ai:providerOptions` は後方互換のための**読み取り専用 legacy 移行ソース**に降格。プロバイダ/API キー（単一）と AI 有効性ゲーティングは不変。
...
- 既定モデルの決定（`isDefault` フラグ、無指定時は先頭）と、`ai:model`/`ai:providerOptions` を legacy 移行ソースとして読む後方互換ロジック。
...
- `ai:allowedModels` の意味・`isDefault` 規約・`ai:model`/`ai:providerOptions` の legacy 扱い変更 → `admin-ai-settings`・`multi-llm-provider` spec 再確認。
...
├── server/routes/
│   └── get-models.ts                    # GET /_api/v3/mastra/models（許可モデル+既定）
└── client/
    ├── admin/
...
### Modified
- `apps/app/src/server/service/config-manager/config-definition.ts` — `ai:allowedModels` 追加、env-only `targetKeys` に追加。`ai:model`/`ai:providerOptions` は legacy 読取専用として定義残置。
...
- `client/components/ChatSidebar/ChatSidebar.tsx` / `chat-sidebar-helpers.ts` — モデルセレクタ mount、transport body に `modelId` を固定（`modelId` 変更時に transport 再生成）、許可モデル取得フック利用。
...
| 1.5 | 既定の妥当性 | put-ai-settings (validator) | exactly-one-`isDefault`（既定は本質的に集合内） | 管理保存 |
...
| 3.2 | 初期=既定 | ChatSidebar | `defaultModelId` | — |
...
| 5.1 | 単一→既定+1エントリ | config (getAllowedModels) | 読取時フォールバック（`isDefault:true` 合成） | — |
...
```
- `ai:model` / `ai:providerOptions` は **deprecated（読取専用 legacy 移行ソース）**: config 定義は残す（env `AI_MODEL` / `AI_PROVIDER_OPTIONS` を読み込めるように）が、`AI_SETTING_KEYS`（管理書込み集合）・管理 UI・PUT から除去し、グローバル適用も廃止。読取時フォールバック合成時のみ参照。
- env-only `targetKeys` に `ai:allowedModels` 追加（`ai:model`/`ai:providerOptions` の legacy エントリも残置）。
...
/** 許可モデル一覧。ai:allowedModels が空かつ legacy ai:model 有のとき
 *  [{ model: ai:model, providerOptions?: <legacy ai:providerOptions>, isDefault: true }] を合成（5.1/5.2）。 */
...
- 認証: login required + 既存 get-threads / get-messages と同じ scope を踏襲（実装時に確認）+ ai-ready ガード。
- `models = getAllowedModels().map(m => ({ id: m.model, name: m.model }))`（フレンドリ名は無いので id=name）。`defaultModelId = getDefaultModel()`。**providerOptions はクライアントへ返さない**（サーバ専用）。
...
- `ai:model: string` / `ai:providerOptions: string`（legacy・読取専用移行ソース）。
...
- `getAllowedModels`: `ai:allowedModels` 有→そのまま / 空かつ `ai:model` 有→`[{model, providerOptions: legacy, isDefault: true}]` 合成（5.1/5.2）。
...
- `GET /_api/v3/mastra/models` が許可モデル+既定を返す（providerOptions 非含、3.1/3.2）。
...
- `ChatSidebar`: セレクタ初期値=既定、選択変更で transport が再生成され `modelId` が送信される / regenerate でも `modelId` が保持される（3.2/3.3/3.4）。
...
graph TB
  ...
  Has -- no --> Model{legacy ai model set}
  Model -- yes --> Synth[synthesize one entry isDefault true with legacy providerOptions]
  Model -- no --> Empty[return empty]
```
- 管理者が新 UI で保存すると `ai:allowedModels` が埋まり、以降フォールバックは通らない（legacy `ai:model`/`ai:providerOptions` も以後無視）。
- ロールバック: `ai:allowedModels` を削除すれば旧単一モデル挙動（legacy `ai:model`+options）に戻る。
