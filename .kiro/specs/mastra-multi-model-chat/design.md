...
- 実行時のベンダー API / レジストリからのモデル一覧自動取得（実行時ベンダー API 呼び出しは引き続き非対象）。（更新: ai-settings-model-picker により、カタログを持つプロバイダの管理者許可リストは、models.dev をリリース前段で取り込んだオフライン同梱カタログ (`model-catalog-data.json`) からの選択方式を採用。一覧提供（read パス）は成果物 read のみで外部通信なし（既定。opt-in のカタログリフレッシュ〔管理画面手動／起動時／定期〕の実行時に限り models.dev へ fetch する）。azure-openai・取得失敗時は従来どおり自由入力。詳細は `.kiro/specs/ai-settings-model-picker`。）
...
- 実行時のベンダー API からのモデル一覧取得は引き続き非対象。ただし ai-settings-model-picker はオフライン同梱（リリース前段で成果物へ commit 済み）カタログの read を採用する（一覧提供 read パスは外部通信ゼロ。opt-in のカタログリフレッシュ〔管理画面手動／起動時／定期〕時のみ models.dev へ fetch する別経路。詳細は `.kiro/specs/ai-settings-model-picker`）。会話固定モデルのサーバ永続化。
