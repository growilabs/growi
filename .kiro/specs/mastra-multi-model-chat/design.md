...
- ユーザー選択モデルの永続化（`UserUISettings.aiChatSelectedModel`、既存 `scheduleToPut`/SSR ハイドレート流用）。
...
- `~/interfaces/user-ui-settings`、`~/client/services/user-ui-settings`（`scheduleToPut`）、SSR `userUISettings` ハイドレート（選択モデル永続化）。
...
└── client/
    ...
    ├── states/
    │   └── selected-model.ts             # 選択モデルの Jotai atom（setter で scheduleToPut→UserUISettings 永続化、SSR ハイドレート）
    └── stores/
...
- `client/components/ChatSidebar/ChatSidebar.tsx` / `chat-sidebar-helpers.ts` — モデルセレクタ mount、transport body に `modelId` を固定（`modelId` 変更時に transport 再生成）、許可モデル取得フック + 選択モデル atom の読み書き（初期選択 = 前回選択 ?? デフォルト、許可リスト検証）。
...
- `apps/app/src/states/ui/sidebar/hydrate.ts`（SSR `userUISettings` 読取箇所）— `aiChatSelectedModel` を選択モデル atom にもハイドレート。
- （再利用・無改変）`components/ai-elements/prompt-input.tsx`、`client/services/user-ui-settings.ts`（`scheduleToPut`）。
...
| 3.2 | 初期=前回選択 ?? 既定 | ChatSidebar, selected-model atom | `IUserUISettings.aiChatSelectedModel` | — |
...
| 3.6 | 選択の永続化 | selected-model atom, user-ui-settings | `scheduleToPut({ aiChatSelectedModel })` | — |
| 3.7 | 永続値が許可外→既定 | ChatSidebar | `find(saved) ?? default` | — |
...
- 初期選択 = 永続化された `aiChatSelectedModel`（atom; 許可リストに在れば）?? `defaultModelId`。`PromptInputModelSelect`（value/onValueChange）+ `PromptInputModelSelectItem` を `models` で map。選択変更で atom を更新し、その setter が `scheduleToPut({ aiChatSelectedModel })` でデバウンス DB 永続化（既存 sidebar atom パターン踏襲）。
...
### ユーザー個人設定（DB）
- `IUserUISettings.aiChatSelectedModel?: string`（`UserUISettings` コレクション、user 単位 unique）。未設定 = 未選択 → 初期選択は既定にフォールバック。グローバル config ではなくユーザー単位。
...
- `ChatSidebar`: セレクタ初期値 = 前回選択（許可内）?? 既定 / 許可外の永続値は既定にフォールバック（3.2/3.7）/ 選択変更で transport 再生成 + `modelId` 送信 + `aiChatSelectedModel` 永続化（`scheduleToPut`）が呼ばれる / regenerate でも `modelId` 保持（3.3/3.4/3.6）。
