...
- `server/services/is-ai-configured.ts` — 構成済み判定を `provider + apiKey + 非空 getAllowedModels()` に更新（5/6 整合）。ただし Azure OpenAI + Entra ID（`useEntraId === true`）は apiKey を要求しない（`requiresApiKey(provider)` で分岐）。
...
| 5.1 | 既存スレッド維持 | （無改変） | — | — |
| 6.1 | AI 無効時チャット不可 | is-ai-configured（更新）, 既存ゲート | `isAiConfigured`（allowedModels ベース。Azure+Entra ID は apiKey 不要）, `isAiReady` | — |
...
- Integration: `growi-agent.ts` の `model` を `({ requestContext }) => resolveMastraModel(requestContext.get('modelId'))` に。各 provider resolver は `create*({apiKey})(model)` の `model` を引数化。`is-ai-configured.ts` は `provider + apiKey + getAllowedModels().length > 0` で判定。**例外**: Azure OpenAI + Entra ID（`ai:azureOpenaiSettings.useEntraId === true`）は apiKey 不要（ベアラートークン認証）のため apiKey を要求しない（`requiresApiKey(provider)` ヘルパが `resolveAzureOpenaiModel` の認証分岐と一致）。これにより Entra 専用デプロイの「従来どおり」ゲーティング（6.1）を保つ。
...
- `isAiConfigured`: provider+apiKey+非空 allowedModels（フォールバック含む）で true / それ以外 false。Azure+Entra ID（`useEntraId=true`）は apiKey 欠落でも true（6.1）。
