...
> 注(mastra-multi-model-chat 反映後): 当初は `ai:model`(単一)/ `ai:providerOptions`(グローバル単一)を含む 5 キーだったが、mastra-multi-model-chat により両者は `ai:allowedModels`(各エントリに model + 任意 providerOptions + isDefault)へ統合・廃止され(env `AI_MODEL` / `AI_PROVIDER_OPTIONS` も廃止・自動移行なし)、現在は下記 4 キー。

| Config Key | 環境変数 | 型 | isSecret |
|---|---|---|---|
| `ai:provider` | `AI_PROVIDER` | `AiProvider \| undefined` | - |
| `ai:apiKey` | `AI_API_KEY` | `string \| undefined` | ✅ |
| `ai:allowedModels` | `AI_ALLOWED_MODELS` | `AllowedModel[] \| undefined`(`{ model; providerOptions?; isDefault? }[]`、JSON 配列、`defaultValue: []`) | - |
...
- provider オプションの解決: `resolve-provider-options.ts` — パース失敗時は警告ログ + `{}` フォールバック(fail-soft)。注(mastra-multi-model-chat 反映後): グローバル単一 `ai:providerOptions` ではなく `resolveProviderOptions(modelId?)` が実効モデルの `ai:allowedModels` エントリから解決する（一律適用は廃止）
...
4. **Research Needed(design で解決)**:
   ...
   - provider オプションのサーバー側バリデーション仕様(JSON 形式チェックのみ / 形状 `Record<string, Record<string, JSONValue>>` まで検証するか)。注(mastra-multi-model-chat 反映後): グローバル単一 `ai:providerOptions` ではなく `ai:allowedModels` の各エントリの `providerOptions` を `isValidProviderOptionsJson` で検証する
...
- `resolveProviderOptions()`(`resolve-provider-options.ts`)は**メモ化していない**(毎回 config を読む)ため、対応不要。注(mastra-multi-model-chat 反映後): 引数 `modelId?` を取り、実効モデルの `ai:allowedModels` エントリから解決する
...
### 設計判断
- `isAiConfigured()`(provider + provider 別必須項目の**非 throw** 検査)+ `isAiReady() = isAiEnabled() && isAiConfigured()` を `features/mastra/server/services/is-ai-configured.ts` に新設。注(mastra-multi-model-chat 反映後): 判定基準は **provider + apiKey + 非空 `ai:allowedModels`** に更新(モデル集合が空なら未構成)。Azure OpenAI + Entra ID(`useEntraId === true`)は apiKey 不要(`requiresApiKey` で分岐)。
