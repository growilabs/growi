...
- **簡素化**: 接続テスト機能は持たない(範囲外)。エンドポイントは GET/PUT 各 1 本。ConfigManager の公開 API(コア `IConfigManager`)は**変更しない** — env 優先は `getConfig` 内部ロジックのみ、env-fixed 判定は route 層で `getManagedEnvVars()` を再利用して算出する
