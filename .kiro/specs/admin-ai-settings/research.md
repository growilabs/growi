...
> 注(mastra-multi-model-chat 反映後): 当初は `ai:model`(単一)/ `ai:providerOptions`(グローバル単一)を含む 5 キーだったが、mastra-multi-model-chat により両者は `ai:allowedModels`(各エントリに modelId + 任意 providerOptions + isDefault)へ統合・廃止され(env `AI_MODEL` / `AI_PROVIDER_OPTIONS` も廃止・自動移行なし)、現在は下記 4 キー。
...
4. **Research Needed(design で解決)**:
   ...
   - provider オプションのサーバー側バリデーション仕様(JSON 形式チェックのみ / 形状 `ModelProviderOptions`(= `Record<string, Record<string, JSONValue>>`、`interfaces/allowed-model.ts` 宣言)まで検証するか)。注(mastra-multi-model-chat 反映後): グローバル単一 `ai:providerOptions` ではなく `ai:allowedModels` の各エントリの `providerOptions` を `isValidProviderOptionsJson` で検証する
...
- `resolve-mastra-model.ts` の **モジュールスコープのメモ** のみ。早期 return するため、`configManager.loadConfigs()` で config 値が更新されても古いモデルを返し続ける(= 中核ギャップ)。注(mastra-multi-model-chat 反映後): メモは `${provider}:${effective}` キーの Map になり、`clearResolvedMastraModelCache()` は Map 全消去
- `getProviderOptionsForModel()`(`resolve-provider-options.ts`)は**メモ化していない**(毎回 config を読む)ため、対応不要。注(mastra-multi-model-chat 反映後): 引数に**実効モデル ID**(`effectiveModelId: string`、解決済み)を取り、その `ai:allowedModels` エントリの `providerOptions` を返す純粋なルックアップ(自前の解決/ラウンディングは行わない)
...
- `app:aiEnabled` を `env:useOnlyEnvVars:ai` グループの対象キーに追加。env 専用モード時はトグルも env 固定・編集不可。注(mastra-multi-model-chat 反映後): グループの対象キーは `app:aiEnabled` + `ai:provider` / `ai:apiKey` / `ai:allowedModels` / `ai:azureOpenaiSettings` の計 5 キー。
