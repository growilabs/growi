...
```
  ...
  'ai:provider',
  'ai:apiKey',
  // 許可モデルの集合。各エントリは { model; providerOptions?; isDefault? }。
  // 既定モデルは別キーではなくリスト内の isDefault エントリ(ちょうど 1 つ)で表す。
  // 旧 ai:model(単一)/ ai:providerOptions(グローバル単一)は mastra-multi-model-chat
  // で完全廃止され、本キーへ統合された(env AI_MODEL / AI_PROVIDER_OPTIONS も廃止)。
  'ai:allowedModels',
...
- **In scope**:
  ...
  - 以下の `ai:*` 設定キーの表示・更新: `ai:provider`, `ai:apiKey`, `ai:allowedModels`(許可モデルの集合。各エントリ = `{ model; providerOptions?; isDefault? }`。既定はリスト内の `isDefault` エントリ。mastra-multi-model-chat で旧 `ai:model` 単一 / `ai:providerOptions` グローバル単一を統合・廃止), `ai:azureOpenaiSettings`(Azure OpenAI 接続設定の 4 論理項目 resourceName / baseURL / apiVersion / useEntraId を 1 つの JSON オブジェクト `AzureOpenaiConfig` として保持し、API でもネストしたオブジェクトとして受け渡す。管理画面は 4 つの入力欄を表示し、各欄はこのオブジェクトのフィールドにバインドする)
  ...
  - 環境変数専用モード(制御用環境変数)が有効なときに AI 設定(上記 `ai:*` 4 キー: `ai:provider`, `ai:apiKey`, `ai:allowedModels`, `ai:azureOpenaiSettings` + `app:aiEnabled`)を環境変数で固定し、UI / API ともに上書き不可とする挙動(既存 `ENV_ONLY_GROUPS` 機構を流用)
- **Out of scope**:
...
- **Adjacent expectations**:
  ...
  - 各許可モデルの `providerOptions`(`ai:allowedModels` の各エントリ)の内容(プロバイダー固有オプションの意味的な妥当性)の検証は各プロバイダー連携側の責務であり、本フィーチャーは JSON としての形式的な妥当性のみを扱う
...
**Objective:** As a GROWI 管理者, I want LLM プロバイダー共通の設定(プロバイダー種別・API キー・許可モデルの集合)を画面から設定したい, so that 環境変数を編集することなく AI 機能を構成できる

#### Acceptance Criteria

1. The AI 設定管理画面 shall `ai:provider`, `ai:apiKey` の各設定項目に対応する入力欄、および `ai:allowedModels`(許可モデルの集合)を編集するリストエディタを提供する。リストエディタの各行はモデル ID(Azure OpenAI ではデプロイメント名)・既定指定(リスト内ちょうど 1 つ)・モデルごとの provider オプション(任意の JSON)から成る。mastra-multi-model-chat に整合し、旧 `ai:model`(単一)/ `ai:providerOptions`(グローバル単一テキストエリア)の入力欄は廃止
...
4. The AI 設定管理画面 shall Azure OpenAI 利用時には許可モデルリスト(`ai:allowedModels`)の各エントリのモデル値がモデル ID ではなくデプロイメント名を指すことを管理者に案内する(リストエディタのラベルを `provider` に応じて「デプロイ名」へ切り替える)
...
2. If 許可モデルリスト(`ai:allowedModels`)のいずれかのエントリの provider オプションに JSON として解釈できない値が入力された場合, the AI 設定管理画面 shall 該当行にバリデーションエラーを表示し、保存を実行しない
