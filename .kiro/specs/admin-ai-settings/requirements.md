...
```
  ...
  // 管理 UI / API では引き続き 4 つの個別フィールドとして公開する。
...
- **In scope**:
  ...
  - 以下の `ai:*` 設定キーの表示・更新: `ai:provider`, `ai:apiKey`, `ai:model`, `ai:providerOptions`, `ai:azureOpenaiSettings`(Azure OpenAI 接続設定の 4 論理項目 resourceName / baseURL / apiVersion / useEntraId を 1 つの JSON オブジェクトとして保持。管理 UI / API では引き続き 4 つの個別フィールドとして公開する)
...
1. The GROWI shall Azure OpenAI 専用設定(resourceName / baseURL / apiVersion / useEntraId の 4 論理項目。これらは単一キー `ai:azureOpenaiSettings` の JSON オブジェクトとして永続化されるが、画面では 4 つの個別フィールドとして公開する)を設定するための専用の設定画面(またはセクション)を提供する
