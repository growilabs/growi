...
- **In scope**:
  ...
  - AI 関連 API の利用可否を「AI が有効 かつ 設定が有効に構成されている」ことに整合させる(設定不備時はリクエストを拒否し、クライアント導線も非表示にする)
  - 環境変数専用モード(制御用環境変数)が有効なときに AI 設定(上記 `ai:*` 8 キー + `app:aiEnabled`)を環境変数で固定し、UI / API ともに上書き不可とする挙動(既存 `ENV_ONLY_GROUPS` 機構を流用)
- **Out of scope**:
...
4. While AI 機能が無効である、またはプロバイダーおよびその必須項目が有効に構成されていない, the GROWI shall (次回のページ読み込み以降)クライアントの AI 機能導線(サイドバー等)を表示しない
