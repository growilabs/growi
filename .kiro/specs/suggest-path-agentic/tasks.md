...
- [ ] 7.1 A/B 測定の実施とメトリクス記録
  ...
  - _Blocked: 意図的な保留（不具合起因ではない）。実 LLM での A/B 測定（6 ユースケース × 10 回 × 2 エンジン = 120 リクエスト）は実トークン消費と測定条件の確認を伴うため、自律モードの対象外とし人間監督下でマニュアル実施する。解除条件: タスク 1〜6 完了後、ユーザーの明示指示により devcontainer で実施_
...
- [ ] 7.2 探索誘導の確認と受け入れ判断
  ...
  - _Blocked: 意図的な保留（不具合起因ではない）。受け入れ判断（改善継続/方針転換）は Requirements 6.4 上も開発チームの意思決定であり、エージェント単独で確定させない。解除条件: 7.1 の測定結果を人間がレビューした後、ユーザーの明示指示によりマニュアル実施_
