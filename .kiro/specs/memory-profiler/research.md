...
- **Context**: `memory-leak-investigation` spec が本 spec の consumer であり、interface 仕様への安定参照を必要とする。
- **Sources Consulted**:
  - [.kiro/specs/memory-leak-investigation/design.md](../memory-leak-investigation/design.md)
  - [.kiro/specs/memory-leak-investigation/tasks.md](../memory-leak-investigation/tasks.md)
- **Findings**:
  - 直前の整理（commit `8be58daca0`）で `memory-leak-investigation` の design / tasks / requirements に本 spec への参照ポイントが用意されている（current HEAD 状態では tool architecture 記述が残存。stash に「reference-only にする clean-up」が保留中）。
  - Investigation spec の Phase 6 タスク（OTel 有効化下計測、Yjs sustained-load、L4 retainer、dist server 計測）はすべて本 spec のツール CLI / env var を通じて完遂可能。
...
- **Context**: `memory-leak-investigation` 等の investigation spec と本 spec の境界をどう設計するか。
- **Alternatives Considered**:
...
- **Rationale**: 本 spec はツールの安定供給を責務とし、特定の調査内容に縛られない。Investigation spec が増えても本 spec の責務は変わらない。
- **Trade-offs**: + 各 investigation spec が独立に増加可能 / − consumer 側の参照漏れに気づく仕組み（change review プロセス）を要求事項にする必要あり。
...
- **CLI / env var / exit code の breaking change**: investigation spec が動作不能になる → change review で必ず downstream への影響評価を要求（design.md と README に明記）。

## References
- [research.md (memory-leak-investigation)](../memory-leak-investigation/research.md) — 同ツールが生まれた背景の研究ノート（歴史的記録、本 spec の出発点）
- [research.md (memory-leak-investigation) Part 2 — Design Decisions](../memory-leak-investigation/research.md#design-decisions) — sidecar / CDP-only 採用の経緯
