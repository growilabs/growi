# Gap Analysis: ai-provider-model-picker
...
### 1.3 型・DTO
- [ai-provider.ts](apps/app/src/features/mastra/interfaces/ai-provider.ts): `AI_PROVIDERS = ['openai','anthropic','google','azure-openai']` + `isAiProvider` 型ガード。冒頭コメントは「Server-only type module … **Do NOT add client imports here**」＝**この中に client 依存を持ち込むな**の意（server-safe な leaf を保つため）であり、client からの利用（被参照）を禁じるものではない。実体は const 配列＋純粋な型ガードのみ（外部 import ゼロ）なので、client からの `import type { AiProvider }`（型のみ・ビルド時に erase）は安全。runtime 値 `isAiProvider` はサーバ（route の provider 検証）でのみ使う。
  - **※ 実装後リファインメント**: 後に `AI_PROVIDERS` は `AI_PROVIDER_DEFS`（provider ごとの `enumerable` メタデータ）から導出する形に変更した。メンバー・振る舞いは不変で、目的は `CATALOG_PROVIDERS` を同フラグから導出して単一ソース化し、`AI_PROVIDERS` との二重管理を排すること。
...
| 8 | 既存スペック整合更新 | `.kiro/specs/ai-provider-multi-model/*`, `.kiro/specs/ai-provider-selection/*` | **New(doc)**: requirements/design/research の該当記述を更新（実装タスクとして） |
...
7. **Req 8 の具体差分**: `ai-provider-multi-model`（requirements 確定判断・design Non-Goals）と `ai-provider-selection`（research D-2/D-3）の**編集箇所を design で特定**し、実装タスク化。
...
- **D7（Req8 具体差分）**: `ai-provider-multi-model`（requirements 確定判断・design Non-Goals）と `ai-provider-selection/research.md`（D-2/D-3）を編集対象として実装タスク化。
...
# カタログの保持先: JSON ファイル vs config（公平な比較, 2026-07-01）

**問い**: モデルカタログのデータを「コミット済み JSON ファイル（X2 の成果物）」で持つのと、「config-manager で管理」するのはどちらが良いか。

## 判断の起点: 「設定」か「参照データ」か
- **設定（config 向き）**: 運用者がインストールごとに決める値（`ai:provider` / `ai:apiKey` / そして**許可リスト `ai:allowedModels`**）。
- **参照データ（アセット向き）**: 上流（models.dev）由来で全インストールで同一・運用者が著すものではない事実。

モデルカタログ（どのモデルが存在し chat+tool 対応か）は**参照データ**の性格。運用者の「このモデルを許可する」選択は既に `ai:allowedModels`（config）が担う。→ カタログと許可リストは別レイヤ。

## 比較表

| 観点 | JSON ファイル（コミット成果物） | config（configManager） |
|---|---|---|
| データ性格との適合 | ◎ 参照データ＝バージョン管理アセット | △ config は運用者設定用でミスマッチ |
| 更新方法 | vendoring script → コミット（差分 PR） | migration/seed・default 埋め込み・runtime fetch(Req2違反) のいずれも劣る |
| install 間の一様性 | ◎ 全 install 同一・git 固定 | △ DB 値は install ごとに差異・バックアップ対象 |
| 変更の可視性/監査 | ◎ PR diff | △ DB 値は不可視／migration は重い |
| **再デプロイ無しの per-install 上書き** | ✕ 不可 | **◎ config の明確な勝ち**（DB/env 差し替え） |
| runtime 可変性 | ✕ 静的 | ◎ 将来「更新ボタン」等が作りやすい |
| DB / config シングルトン負荷 | ◎ 無関係 | △ 全 config を起動時ロード、blob は不適 |
| GROWI 既存流儀 | ◎ emoji/marpit/prisma＝生成物アセット | △ カタログ級参照データを config に置く前例なし |
| 攻撃面/検証負担 | ◎ 小 | △ 設定 API 経由編集の検証が要る |

## config が勝つ唯一の軸とその現実
config の強みは **「運用者が再デプロイなしで install ごとにカタログを差し替えられる」**点（エアギャップ/独自エンドポイント向け）。ただし GROWI では:
- 「どのモデルを許可するか」の per-install キュレーションは **`ai:allowedModels`（config）が既に担当**。
- Azure/独自/私設モデル ID は **azure＝自由入力**で対応済み。

→ config の勝ち筋は**カタログ本体で担う必要がなく、既存の許可リスト＋自由入力でほぼ満たされる**。カタログを config に移す動機は薄い。

## 結論（公平版）
- **カタログ本体は JSON ファイル（コミット成果物）が適合**: 参照データ・全 install 一様・上流から再生成・PR で可視・GROWI 流儀（emoji/marpit）一致・DB/シングルトンを汚さない。
- **config は「運用者の選択」= `ai:allowedModels` に使い続ける**（不変）。
- config の唯一の勝ち筋（per-install 動的上書き）が要件化したら、**カタログを config 主保管に移すのではなく、JSON を既定にしつつ任意 `ai:modelsCatalogOverride`（config/env）を1つ足す hybrid** で取り込む（config の利点だけを弊害なしに）。今回はスコープ外（薄く保つ判断）。

**決定ルール**:
- カタログが「全 install 同一・上流由来の参照データ」である限り → **JSON ファイル**。
- 「各 install で運用者がカタログ自体を UI/env から動的差し替え」が要件化した時のみ → **config override を追加**（主保管は JSON のまま）。

現状要件では前者 → **JSON ファイルを維持**。config は無意味ではなく per-install 上書きの一点で優れるため、そのニーズ発生時に override で取り込むのが筋。

---

# 追補: カタログに公式表示名 `name` を追加（2026-07-10, imprv/187026）

UI でプロバイダー名・モデル名を正式名称で表示する改善（187026）に伴い、本 spec が定義したカタログ／DTO の shape を後方から更新した。**本節が最終**であり、上記の各節（特に As-Is 調査・D3・チャット側前例の記述）で `string[]` / `modelIds` / 「bare id」/「表示名＝素の ID」とある箇所はこの追補が優先する。

- **カタログ shape**: `provider → string[]`（bare id）→ `provider → { id: string; name: string }[]`（`ModelCatalogEntry`）。`name` は models.dev の `name` を生成時に id と同一スナップショットから取得（相互 drift なし）、欠落時は id をフォールバック。同梱 JSON と永続スナップショット（`mastra_refreshed_model_catalog.models` / `persistedModelCatalogSchema`）の双方に適用。
- **アクセサ改名（いずれも `{id,name}[]` を返す）**: `getSelectableModelIds`→`getSelectableModels`、`getEffectiveSelectableModelIds`→`getEffectiveSelectableModels`、`pickSelectableModelIds`→`pickSelectableModels`。
- **DTO**: `SelectableModelsResponse` は `{ modelIds: string[] }`→`{ models: SelectableModel[] }`（`SelectableModel = {id,name}`）。`GET /ai-settings/available-models` の応答も `{ models }`（D3 の `{ modelIds }` を置換）。
- **表示名の解決（新規）**: 許可リスト（config）は id のみ保持（不変）。表示名は読み取り時に共有ヘルパー `buildModelDisplayNameResolver` が (provider, modelId) を実効カタログと join して解決（id フォールバック）。`ChatModelEntry` に `displayName`、admin GET 応答の許可リスト各要素に表示専用 `displayName`（`AllowedModelForDisplay`。PUT では送らない）を追加。プロバイダー表示名は `AI_PROVIDER_DEFS[p].label` / `getProviderLabel(provider)`。
- **不変**: 環境変数 `AI_ALLOWED_MODELS`（`ai:allowedModels`）の形式（id のみ）、オフライン read 保証、`isSelectableModel` フィルタ規則、azure-openai の自由入力（非 enumerable）。
