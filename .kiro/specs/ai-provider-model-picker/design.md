...
- `.kiro/specs/ai-provider-multi-model/{requirements.md,design.md,research.md}` / `.kiro/specs/ai-provider-selection/research.md` — 8.x の整合更新（実装タスクとして）。
...
- ランタイムのデータ移行なし。8.x はドキュメント整合更新（実装タスク）:
  - `ai-provider-multi-model`: 「モデル一覧 API 新設しない／管理者手入力」→「オフライン vendored カタログの選択方式を採用」に更新（8.1）。
  - `ai-provider-selection`: research D-2/D-3 に「models.dev の **runtime fetch（モデルルーター）は不採用のまま**／**取り込みステップ（リリース前段）で vendoring した静的カタログの read は別物**・推論は native `@ai-sdk/*`」を注記（8.2）。
