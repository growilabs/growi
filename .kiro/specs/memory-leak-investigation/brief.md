...
- 本 spec 完了時点で `research.md` + 新規 `verification-report.md` の 2 文書が成果物として残り、将来のメモリ調査で再利用できる profiling 手順がドキュメント化されている。
...
1. **Phase 1: Dynamic profiling 基盤の構築**
   ...
   - Snapshot 取得: **CDP (Chrome DevTools Protocol)** 経由の WebSocket sidecar が `HeapProfiler.takeHeapSnapshot` を発行し `.heapsnapshot` を保存。フォールバックとして `v8.writeHeapSnapshot(filename)` を SIGUSR2 ハンドラで in-process に発火可能にする。
   - Load driver: `undici` ベースの Node スクリプト 1 本。`/api/v3/installer/` で admin 作成 → page create/edit/delete → y-websocket session の open/close を反復する。`autocannon` は補助的に RSS の時系列観測用に併用する。
   - 出力先: `tmp/memory-leak-investigation/snapshots/{phase}/*.heapsnapshot` および `tmp/memory-leak-investigation/rss-timeseries.csv`。`tmp/` は .gitignore 済。
...
3. **Phase 3: Confirmed finding に対する fix の実装**
   - **L1** (確定): `mongoOptions` に `maxPoolSize: Number(process.env.MONGO_MAX_POOL_SIZE ?? 10)` / `minPoolSize: Number(process.env.MONGO_MIN_POOL_SIZE ?? 2)` を追加。
...
- **Out**:
  ...
  - 一般化された profiling フレームワーク（`@growi/profiling` のような新規パッケージ化）。本 spec ではあくまでこの調査向けのスクリプト群を残すに留める。

## Boundary Candidates
1. **Profiling infrastructure** — CDP sidecar / load driver / scenario runner。`bin/memory-profiling/`（`@growi/bin` workspace package）に集約。

   > 当初は `tools/memory-profiling/` ディレクトリ配置 + SIGUSR2 in-process fallback を予定していたが、実装中に `bin/memory-profiling/`（独立 workspace package）への移動 + CDP-only 方式 に変更した（commit `b8e3efa4c7`）。
2. **Baseline-bloat fixes (L1 + L2)** — `mongoose-utils.ts` の pool size 設定と `node-sdk-configuration.ts` の instrumentation allow-list 化。両方とも 1 ファイル局所の修正で、互いに独立。
3. **Leak-surface instrumentation (L3 metric)** — `features/opentelemetry/server/custom-metrics/yjs-metrics.ts` の新規モジュール（`setupCustomMetrics()` に組み込む）。
4. **Leak mitigation (L3 sweeper + L4 backpressure + L5 try/catch)** — Phase 2 の dynamic 結果が問題量を示した場合のみ実装する条件付き作業。
5. **Verification report** — `research.md` の各 finding に対する裏付け結果と数値根拠を `verification-report.md` にまとめる。
...
- **Downstream**:
  - 将来の memory-leak 調査や capacity planning。本 spec で残す profiling スクリプト・`verification-report.md` の手順をテンプレートとして再利用できる。
...
- **新規 npm dependency の追加は最小限**。`undici` / `autocannon` / `heapsnapshot-parser` 等は profiling スクリプト用に dev dependency として追加する程度に留める。Production runtime には dependency 追加しない。
- **Cross-platform**: profiling スクリプトは devcontainer (Linux) 前提でよい。Windows / macOS native での実行は scope 外。
