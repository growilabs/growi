...
**Users**:
- メモリ調査担当者 — `apps/app/tools/memory-profiling/` のスクリプト群を使って Baseline / Load / Drain シナリオを実行し、`.heapsnapshot` を取得・diff する。
...
**Impact**: server コードを 4 ファイル局所修正 + 1 新規 custom-metrics module + 1 SIGUSR2 ハンドラ追加。`tools/memory-profiling/` ディレクトリを新規作成。Default 値の変更（pool size、auto-instrumentation 範囲）は release notes / CHANGELOG で明示する。
...
- `apps/app/tools/memory-profiling/` 配下に再利用可能な profiling スクリプトとシナリオを残す。
...
### This Spec Owns
- `apps/app/tools/memory-profiling/` 配下の profiling 関連スクリプト一式（CDP sidecar、load driver、シナリオ定義、RSS time-series logger、README）。
- `apps/app/src/server/util/heap-snapshot-handler.ts`（新規。SIGUSR2 で `v8.writeHeapSnapshot()` を起動する hook、`MEMORY_PROFILING_ENABLED` env var でガード）。
...
- Node.js 標準: `node:v8` (`writeHeapSnapshot`), `node:inspector`, `node:process` (signal handler) — runtime dependency 追加なし。
...
- Node.js の major version upgrade — `v8.writeHeapSnapshot` / SIGUSR2 / inspector の挙動を再確認。
...
- **Process signal hook の典型箇所** — [apps/app/src/server/app.ts](../../apps/app/src/server/app.ts) の boot シーケンス末尾。SIGTERM / SIGINT のグレースフルシャットダウンが既に登録されている。SIGUSR2 は env var ガード付きで同レイヤに追加可能。

### Architecture Pattern & Boundary Map

採用パターン: **External profiling sidecar + minimal server-side hook + config-driven baseline tuning**。

```mermaid
graph TB
    subgraph DevcontainerEnv[Devcontainer Environment]
        subgraph ServerProcess[GROWI Server Process node --inspect]
            HeapHook[heap-snapshot-handler SIGUSR2 gated by env var]
        ...
        subgraph SidecarProcess[Profiling Sidecar Process]
...
- **Selected pattern**: External sidecar + signal hook + config-driven baseline tuning（[research.md (Part 2) — Design Decisions](./research.md#design-decisions) 参照）。
- **Domain/feature boundaries**: profiling tooling は `apps/app/tools/memory-profiling/` に隔離し、server runtime とは別プロセスで動く。server 側の追加面は env var でガードされた 1 ファイル（heap-snapshot-handler.ts）と既存 4 ファイルへの局所修正のみ。
...
- **New components rationale**:
  ...
  - `heap-snapshot-handler.ts`: in-process snapshot 取得のフォールバック手段（CDP 不可時）。env var ガードで production 影響なし。
  - `tools/memory-profiling/`: 再利用可能な profiling 手段の集約。
- **Steering compliance**:
...
|-------|------------------|-----------------|-------|
| Backend / Services | Node.js ^24 (existing) | Server runtime、SIGUSR2 hook、`v8.writeHeapSnapshot` | 既存 runtime。追加なし。 |
...
│   │   │   └── heap-snapshot-handler.ts             # [NEW] SIGUSR2 -> v8.writeHeapSnapshot, env-gated
│   │   ├── app.ts                                   # [MODIFY] heap-snapshot-handler の登録
│   │   └── service/
...
├── tools/
│   └── memory-profiling/                            # [NEW] profiling sidecar 一式
...
└── .env.development                                 # [MODIFY] MEMORY_PROFILING_ENABLED 例の追記
...
├── tasks.md                                         # 次フェーズで生成
└── verification-report.md                           # [NEW: Phase 4] 検証結果の集約
```
...
- `apps/app/src/server/app.ts` — `MEMORY_PROFILING_ENABLED` 環境変数が truthy のときのみ `heap-snapshot-handler` を import + 登録。
- `apps/app/.env.development` — `MEMORY_PROFILING_ENABLED` のコメントアウト例を追記（実体は false）。

> 各ファイルは 1 つの明確な責務を持つ。`tools/memory-profiling/` の各ファイルは「シナリオ orchestration」「CDP 通信」「HTTP 駆動」「yjs 駆動」「RSS ロギング」の 5 つの分離されたモジュールで構成される。
...
sequenceDiagram
    ...
    Op->>Server: node --inspect dist/server/app.js (MEMORY_PROFILING_ENABLED=true)
    Server-->>Server: register SIGUSR2 handler
    Server->>Cdp: open inspector endpoint :9229
    Op->>Sidecar: tsx tools/memory-profiling/run-scenario.ts
...
**Key flow decisions**:
- CDP 経由が primary。SIGUSR2 は CDP 不可時の fallback で、`kill -USR2 <pid>` で同じ出力先に snapshot を書く。両者の出力ファイル名規約は同じ。
...
|-------------|---------|------------|------------|-------|
| 1.1 | profiling モード起動 | HeapSnapshotSignalHandler, ScenarioRunner | env var contract (`MEMORY_PROFILING_ENABLED`) | Profiling Session Sequence (boot) |
| 1.2 | CDP 経由 snapshot 保存 | CdpSnapshotClient | CDP `HeapProfiler.takeHeapSnapshot` | Profiling Session Sequence (snapshot) |
| 1.3 | SIGUSR2 in-process fallback | HeapSnapshotSignalHandler | `process.on('SIGUSR2')` -> `v8.writeHeapSnapshot` | — |
| 1.4 | tmp ディレクトリ配下集約 | ScenarioRunner, RssTimeSeriesLogger | Output path = `tmp/memory-leak-investigation/` | Profiling Session Sequence (write) |
| 1.5 | snapshot 失敗時の非影響 | CdpSnapshotClient, HeapSnapshotSignalHandler | try/catch + logger | — |
...
| 6.4 | 手順ドキュメント化 | `tools/memory-profiling/README.md` | README structure | — |
...
| HeapSnapshotSignalHandler | Server / Diagnostics | SIGUSR2 で `v8.writeHeapSnapshot` を起動 (env-gated) | 1.1, 1.3, 1.4, 1.5 | `node:v8`, `node:process` (P0) | Event (signal) |
...
#### HeapSnapshotSignalHandler

| Field | Detail |
|-------|--------|
| Intent | SIGUSR2 受信で `v8.writeHeapSnapshot()` を起動し、CDP 不可時の fallback を提供 |
| Requirements | 1.1, 1.3, 1.4, 1.5 |

**Responsibilities & Constraints**
- `MEMORY_PROFILING_ENABLED` が truthy のときのみ `process.on('SIGUSR2', ...)` を登録。それ以外では一切何もしない（production 影響ゼロ）。
- 出力先は `MEMORY_PROFILING_OUTPUT_DIR` env var（default: `tmp/memory-leak-investigation/snapshots/`）。
- ファイル名規約: `signal-{ISO8601-timestamp}.heapsnapshot`。
- 例外を server プロセスに伝播しない（要件 1.5）。

**Dependencies**
- External: Node `node:v8` `writeHeapSnapshot`, `node:process` signal handler API (P0)
- External: `~/utils/logger`（catch ログ用） (P1)
- Inbound: `apps/app/src/server/app.ts` boot シーケンスから 1 回 import + 呼び出し
...
```
- Preconditions: `MEMORY_PROFILING_ENABLED` env var が truthy。
- Postconditions: SIGUSR2 受信時に snapshot ファイルが出力ディレクトリへ書き込まれる。
- Invariants: production（env var 未設定時）では signal handler を登録しない。

**Implementation Notes**
- Integration: `app.ts` の boot 最後で `if (process.env.MEMORY_PROFILING_ENABLED) { registerHeapSnapshotSignalHandler(); }`。
- Validation: 出力ディレクトリ未存在時は `fs.mkdirSync(..., { recursive: true })`。
- Risks: SIGUSR2 と Node debugger の競合（[research.md Part 2 / Risk R1](./research.md#risks--mitigations)）→ env var ガードで production 影響なし、devcontainer での smoke で確認。
...
**Implementation Notes**
- Integration: エントリポイント `tools/memory-profiling/run-scenario.ts` から CLI 引数 / env var を解釈して `runScenario(opts)` を呼ぶ。
...
- Inbound: `tools/memory-profiling/scenarios/load.ts` から op ごとの回数を渡して呼ばれる
...
- **Server-side hook のエラー**: `heap-snapshot-handler.ts` の SIGUSR2 callback 内例外は try/catch で吸収し `logger.error` で出力（server 本体は停止させない、要件 1.5）。
...
5. **HeapSnapshotSignalHandler**: env var 未設定で `registerHeapSnapshotSignalHandler` を呼んでも signal listener が追加されないこと。env var 設定時は SIGUSR2 を `process.emit` した際に `v8.writeHeapSnapshot` が呼ばれること（`node:v8` を mock）。
...
### E2E / 手動シナリオ
1. **Profiling session 1 周回し**: devcontainer で `node --inspect dist/server/app.js` を起動し `tsx tools/memory-profiling/run-scenario.ts` を実行 → `tmp/memory-leak-investigation/` に snapshot A/B/C + rss-timeseries.csv が生成されることを目視で確認。
...
4. **SIGUSR2 fallback**: `MEMORY_PROFILING_ENABLED=true` 起動後 `kill -USR2 $(pidof node)` で snapshot ファイルが生成されることを確認。
...
- **Inspector エンドポイント (`:9229`) は devcontainer ローカルでのみ listen** する。production では `--inspect` を付けない。`MEMORY_PROFILING_ENABLED` env var も production では未設定。
- **`tmp/memory-leak-investigation/*.heapsnapshot` には機密情報を含む可能性** — devcontainer ローカル開発の admin user / page content が snapshot に乗る。リポジトリにコミットしない、共有しない、不要になり次第削除する旨を README に記載。
