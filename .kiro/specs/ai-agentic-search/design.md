...
export const growiAgent = new Agent({
  ...
  // - Use the fileSearch tool when the question relates to the user's wiki content.   // disabled: see spec ai-agentic-search
  ...
  tools: {
    // fileSearchTool, // disabled: see spec ai-agentic-search
    fullTextSearchTool,
    getPageContentTool,
  },
  memory,
...
5c. `totalCount` は `formatted.meta.total` 由来であること（`searchKeyword` 生結果の `meta` や `hits.length` ではない。formatter は Mongo 側で削除済みのページを drop するため `hits.length < totalCount` が正当な状態として存在する）（6.2）
