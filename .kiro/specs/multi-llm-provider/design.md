...
- 選択パターン: **データ駆動の provider 解決**（`LLM_PROVIDERS` 配列＋provider→自己解決関数 `() => MastraModelConfig` のマップ `modelResolvers`）。consumer は provider 名で分岐しない。
...
- `apps/app/src/features/mastra/server/services/ai-sdk-modules/llm-providers/index.ts` — barrel。`modelResolvers: Record<LlmProvider, () => MastraModelConfig>`（provider → 自己解決関数のマップ）を組み立てて export するのみ。
...
**Implementation Notes**
- Integration: 1 App = 1 Vendor のため**単一キーセット**。provider は `mastra:llmProvider` で選択し、resolver が `modelResolvers[provider]()` を呼ぶ。`openai:apiKey` 等の既存キーは suggest-path 用に不変（mastra は参照しない）。
...
```typescript
// config.ts — 共有アクセサ
export const requireApiKey = (): string => { /* mastra:llmApiKey、欠落で throw */ };
export const getModel = (): string => configManager.getConfig('mastra:llmModel');
...
export const modelResolvers: Record<LlmProvider, () => MastraModelConfig> = {
...
- Invariants: resolver 自身は provider 値のみ読む（apiKey/model/azure config の読取は各 provider の resolver へ移譲）。throw メッセージに API キー値を含めない（Req 2.5）。per-vendor 既定モデル map は持たない（既定は config の `mastra:llmModel` defaultValue に集約）。

解決手順:
1. memoize 済みなら即返す（Req 3.1）。
2. `mastra:llmProvider` 取得（config default `'openai'`。未指定時は OpenAI。Req 1.3）。
3. `isLlmProvider` 失敗なら throw（不正 provider 名を含むメッセージ。untrusted env を弾く。Req 1.4）。
...
3. **データ駆動ディスパッチ（consumer は provider 非依存）**: `modelResolvers: Record<LlmProvider, () => MastraModelConfig>` を barrel で組み立て、resolver は `modelResolvers[provider]()` を呼ぶだけ。**provider 追加で resolver（consumer）は不変** → `.claude/rules/coding-style.md`「Data-Driven Control over Hard-Coded Mode Checks」に準拠。
...
export const getApiKey = (): string | undefined => configManager.getConfig('mastra:llmApiKey');
export const requireApiKey = (): string => { /* 欠落で throw（MASTRA_LLM_API_KEY 案内・キー値非含） */ };
export const getModel = (): string => configManager.getConfig('mastra:llmModel');
...
```
azure の検証/排他/認証ルール（`resolveAzureOpenaiModel` 内）:
- 検証: `resourceName == null && baseURL == null` なら throw（`MASTRA_LLM_AZURE_OPENAI_RESOURCE_NAME` / `MASTRA_LLM_AZURE_OPENAI_BASE_URL` を名指し・キー値非含。Req 7.4）。
...
if (!isLlmProvider(provider)) throw Unsupported...;   // Req 1.4（未検証 env を弾く）
...
| 7.1, 7.2 | Azure エンドポイントを env 指定（リソース名/ベース URL 両対応） | config, azure resolver | `mastra:llmAzureOpenai*` | リクエスト時供給 |
...
| 7.5 | apiVersion 任意（未指定→SDK 既定） | config, azure resolver | `mastra:llmAzureOpenaiApiVersion` | — |
| 7.6 | Azure では model=デプロイ名 | azure resolver | `getModel()` を deployment として渡す | — |
| 8.1, 8.2 | API キー / Entra ID の 2 方式を env フラグで選択（既定 API キー） | config, azure resolver | `mastra:llmAzureOpenaiUseEntraId`（default false） | リクエスト時供給 |
...
- **modelResolvers map**: `Object.keys` が `LLM_PROVIDERS` と一致／各 key が対応 resolver に振り分け。
