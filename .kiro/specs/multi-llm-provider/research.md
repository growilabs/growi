...
- **Generalization**: R1–R5 は「ベンダーに依存しない LLM モデル解決」という単一問題の変奏。`vendor → LanguageModel` を返す**純関数リゾルバ 1 つ**にデータ駆動（`LLM_VENDORS` 配列＋ per-vendor factory map）で集約し、consumer（agent/route）はベンダー名で分岐しない（coding-style の data-driven 原則）。
...
1. **`mastra:llmVendor` を型付け**（config-definition レベル）: `string | undefined` → **inline literal union `'openai' | 'anthropic' | 'google'`**。`LlmVendor` を import せず（core→feature 依存逆転を回避、`openai:serviceType` と同流儀）。
2. **`mastra:llmVendor` default を `'openai'`** に: Req 1.3 を「明示必須・フォールバックなし」→「未指定時は既定 OpenAI」に反転（当初の私の推奨に回帰）。resolver の vendor-unset throw は撤去。
...
**重要な結論（`isLlmVendor` の要否）**: config-definition で型を付けても `isLlmVendor` は**不要にならない**。config-manager は env 文字列を宣言型で検証しないため、`MASTRA_LLM_VENDOR=azure` は型上 union でも実行時に通り、`llmModelFactories['azure']` undefined でクラッシュする。Req 1.4 のため resolver の実行時検証が必須（`openai:serviceType` も型 union＋`OpenaiServiceTypes.includes` の二段構え）。

検証: feature テスト 79 passed、typecheck は既存 post-message.ts:77 のみ。

---

# D-12. FB 追加: LlmVendor 型の共有 + 未使用 openai キー削除（2026-06-10）

1. **`mastra:llmVendor` を共有 `LlmVendor` で型付け**: inline literal `'openai'|'anthropic'|'google'` をやめ、`import type { LlmVendor }` で単一ソース化。**type-only import は実行時に消える**ため core→feature の runtime エッジは無く、`llm-vendor.ts` は依存ゼロの leaf なので循環もない（D-11 で懸念した依存逆転は type-only なら実害なし）。`isLlmVendor` の実行時検証は引き続き必須（型は実行時強制でない）。
