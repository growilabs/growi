...
- **Adjacent expectations**: 配信側は本文を Markdown 文字列として body に格納し、画像をフィードと同一オリジンの `images/` ディレクトリ(feed.json と同ディレクトリ直下)に置いて `images/foo.gif` の相対パスで参照する。配信側・アプリ側のどちらが先にリリースされても他方を壊さない
...
2. The /_news ページ shall 解決後の URL が https であり、かつ配信フィードと同一オリジンの `images/` ディレクトリ配下を指す場合のみ、その画像を表示する
...
3. The /_news ページ shall 描画パイプラインでの二段検証(URL 解決プラグインによる同一オリジン封じ込め + sanitize)を適用し、メディア URL が https でない、または同一オリジンの `images/` 配下でない場合、その画像を表示しない
