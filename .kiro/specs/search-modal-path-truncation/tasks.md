...
- タスク4（実機ビジュアル検証）: runtime smoke boot は PASS（`turbo run dev` でアプリ起動・コンパイル成功、クライアント変更による crash なし）。当 devcontainer は現在は全文検索が有効（`ELASTICSEARCH_URI` 設定済み・Elasticsearch 稼働・`growi` インデックスあり）で、検索モーダルの実データ目視が可能。深い階層・長いセグメント・日付パス（例 `/alpha/beta/gamma/delta`、6 セグメントの長い CJK/英字パス、`/sandbox/diary/2026/07/22`、`/blog/2026/2026/07/22`）をシードし、共通語で検索して**ライトテーマで 1 行表示・中間省略・日付束ね・長いセグメントの ellipsis 切り詰めが崩れず表示されること**を確認済み。→ 残りの確認（**ダークテーマ**での崩れ／ホバーでの `title` フルパス表示／結果クリックの遷移）は未記録のため、チェックボックスは未チェックのまま。
